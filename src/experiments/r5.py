...
R5_TRUE_EQUAL_THRESHOLD = 0.80
R5_CLOSER_THRESHOLD = 0.80
...
def summarize_r5_oracle_map(
    ...
    for environment in environments:
        ...
        summary["r5_joint_oracle_candidate"] = 1.0 if (
            float(summary["oracle_true_equal_outcome_rate"]) >= R5_TRUE_EQUAL_THRESHOLD
...
def evaluate_r5_rr_environment(
    environment: str,
    config: EnvironmentConfig,
    n_episodes: int,
...
def summarize_r5_rr_environments(
    episode_rows: Sequence[Mapping[str, float | int | str]],
) -> List[Dict[str, float | int | str]]:
    ...
    for environment in environments:
        ...
        summary["r5_joint_discovery_candidate"] = 1.0 if (
            float(summary["true_equal_outcome_rate"]) >= R5_TRUE_EQUAL_THRESHOLD
            and float(summary["closer_to_true_equal_outcome_than_equal_split_rate"])
            >= R5_CLOSER_THRESHOLD
        ) else 0.0
        summary["r5_joint_confirmed"] = 1.0 if (
            float(summary["true_equal_outcome_one_sided_95_low"]) >= R5_TRUE_EQUAL_THRESHOLD
...
def evaluate_r5_fixed_sampling_budgets(
    environment: str,
    config: EnvironmentConfig,
    n_episodes: int,
...
def summarize_r5_fixed_sampling_budgets(
    episode_rows: Sequence[Mapping[str, float | int | str]],
) -> List[Dict[str, float | int | str]]:
