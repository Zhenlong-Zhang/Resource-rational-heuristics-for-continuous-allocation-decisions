...
try:
    ...
    from policies.heuristic import EqualSplitBaselinePolicy, ManualActiveSearchEqualOutcomePolicy
    from experiments.active_search_evaluation import (
        deterministic_realized_utility,
        evaluation_episode_fingerprint,
        full_information_oracle_metrics,
        full_information_utilitarian_allocation,
...
STRATEGY_MAPPING_POLICY_RR = "frozen_rr"
STRATEGY_MAPPING_POLICY_MANUAL = "manual_active_search_equal_outcome"
STRATEGY_MAPPING_POLICY_EQUAL_SPLIT = "equal_split"
STRATEGY_MAPPING_POLICY_ORACLE = "full_information_oracle"
...
STRATEGY_MAPPING_DEFAULT_GAP_BIN_EDGES = (0.0, 10.0, 20.0, 40.0, 80.0, math.inf)
STRATEGY_MAPPING_DEFAULT_TOTAL_NEED_BIN_EDGES = (-math.inf, 0.0, 50.0, 100.0, 150.0, math.inf)
STRATEGY_MAPPING_FIXED_TOTAL_NEED_MEAN = 35.0
STRATEGY_MAPPING_FIXED_TOTAL_NEED_DIFFERENCES = (0.0, 10.0, 20.0, 40.0, 60.0)
STRATEGY_MAPPING_ALLOCATION_TIE_TOLERANCE = 1e-9
STRATEGY_MAPPING_POLICY_SEED_OFFSET = 300_000
...
def ambiguous_close_true_equal_but_closer_equal_split(
    ...
    tie_tolerance: float = STRATEGY_MAPPING_ALLOCATION_TIE_TOLERANCE,
) -> float:
...
def _policy_episode_row(
    ...
    policy_seed = (config.random_seed or 0) + STRATEGY_MAPPING_POLICY_SEED_OFFSET + episode.episode_index * 17
...
def _oracle_row(
    ...
    row: Dict[str, object] = {
        ...
        + STRATEGY_MAPPING_POLICY_SEED_OFFSET
...
def evaluate_strategy_mapping_four_way_environment(
    environment: str,
    config: EnvironmentConfig,
    ...
    execution_order: Sequence[str] = STRATEGY_MAPPING_POLICY_ORDER,
) -> List[Dict[str, object]]:
    """Evaluate four frozen strategies on identical held-out episodes."""

    if not evaluation_episodes:
        return []
    if tuple(sorted(execution_order)) != tuple(sorted(STRATEGY_MAPPING_POLICY_ORDER)):
        raise ValueError("execution_order must contain each strategy mapping policy exactly once")
    ...
    for episode in evaluation_episodes:
        if episode.observation_streams is None:
            raise ValueError("strategy mapping requires pre-generated common observation streams")
        episode_rows: Dict[str, Dict[str, object]] = {}
        for policy_name in execution_order:
            if policy_name == STRATEGY_MAPPING_POLICY_RR:
            ...
            elif policy_name == STRATEGY_MAPPING_POLICY_MANUAL:
                episode_rows[policy_name] = _policy_episode_row(
                    environment,
                    config,
                    episode,
            ...
            elif policy_name == STRATEGY_MAPPING_POLICY_EQUAL_SPLIT:
                episode_rows[policy_name] = _policy_episode_row(
                    environment,
                    config,
                    episode,
        ...
        rr_row = episode_rows[STRATEGY_MAPPING_POLICY_RR]
        manual_row = episode_rows[STRATEGY_MAPPING_POLICY_MANUAL]
        split_row = episode_rows[STRATEGY_MAPPING_POLICY_EQUAL_SPLIT]
        oracle_row = episode_rows[STRATEGY_MAPPING_POLICY_ORACLE]
...
def _validate_four_way_common_randomness(rows: Sequence[Mapping[str, object]]) -> None:
    ...
    for key, episode_rows in grouped.items():
        if len(episode_rows) != 4 or {str(row["policy"]) for row in episode_rows} != set(
            ...
            raise RuntimeError(f"strategy mapping policy-row mismatch for {key}")
        for field in (
            "need_1",
            "need_2",
            "episode_fingerprint",
            ...
            if len({row[field] for row in episode_rows}) != 1:
                raise RuntimeError(f"strategy mapping common-randomness mismatch for {key}: {field}")
...
def summarize_strategy_mapping_four_way(
    ...
    comparisons = (
        ("rr_minus_manual", STRATEGY_MAPPING_POLICY_RR, STRATEGY_MAPPING_POLICY_MANUAL),
        ("rr_minus_equal_split", STRATEGY_MAPPING_POLICY_RR, STRATEGY_MAPPING_POLICY_EQUAL_SPLIT),
        ("manual_minus_equal_split", STRATEGY_MAPPING_POLICY_MANUAL, STRATEGY_MAPPING_POLICY_EQUAL_SPLIT),
        ("oracle_minus_rr", STRATEGY_MAPPING_POLICY_ORACLE, STRATEGY_MAPPING_POLICY_RR),
    )
    ...
    for environment in environments:
        ...
        for dimension, stratum, stratum_rows in _strata(environment_rows):
            ...
            rr_rows = by_policy[STRATEGY_MAPPING_POLICY_RR]
            manual_rows = by_policy[STRATEGY_MAPPING_POLICY_MANUAL]
            split_rows = by_policy[STRATEGY_MAPPING_POLICY_EQUAL_SPLIT]
            ...
            paired_summaries.append(
                {
                    ...
                    "positive_policy": STRATEGY_MAPPING_POLICY_RR,
                    "negative_policy": STRATEGY_MAPPING_POLICY_MANUAL,
...
def _attach_recovery_classifications(
    ...
    for environment in environments:
        ...
        rr = all_policy[STRATEGY_MAPPING_POLICY_RR]
        manual = all_policy[STRATEGY_MAPPING_POLICY_MANUAL]
        oracle = all_policy[STRATEGY_MAPPING_POLICY_ORACLE]
        ...
        fields: Dict[str, object] = {
            ...
            "strategy_mapping_recovery_classification": classification,
        }
        for row in policy_summaries:
...
def build_strategy_mapping_sigma_need_configs(
    base_name: str,
...
def evaluate_strategy_mapping_sigma_need_sweep(
    base_name: str,
    base_config: EnvironmentConfig,
    sigma_need_values: Sequence[float],
    n_episodes: int,
    ...
    rows: List[Dict[str, object]] = []
    for environment, config in build_strategy_mapping_sigma_need_configs(
    ...
    for episode_index, episode_rows in grouped.items():
        for field in (
            ...
            if max(values) - min(values) > 1e-10:
                raise RuntimeError(
                    f"strategy mapping standardized-need mismatch for episode {episode_index}: {field}"
                )
        for field in ("observation_residual_hash_1", "observation_residual_hash_2"):
            if len({row[field] for row in episode_rows}) != 1:
                raise RuntimeError(
                    f"strategy mapping standardized-observation mismatch for episode {episode_index}: {field}"
                )
...
def summarize_strategy_mapping_sigma_need_sweep(
    episode_rows: Sequence[Mapping[str, object]],
    gap_bin_edges: Sequence[float] = STRATEGY_MAPPING_DEFAULT_GAP_BIN_EDGES,
    total_need_bin_edges: Sequence[float] = STRATEGY_MAPPING_DEFAULT_TOTAL_NEED_BIN_EDGES,
) -> Tuple[List[Dict[str, object]], List[Dict[str, object]]]:
...
def evaluate_strategy_mapping_fixed_total_need_diagnostic(
    environment: str,
    base_config: EnvironmentConfig,
    n_episodes_per_difference: int,
    rr_policy: MetaPolicy,
    total_need_mean: float = STRATEGY_MAPPING_FIXED_TOTAL_NEED_MEAN,
    need_differences: Sequence[float] = STRATEGY_MAPPING_FIXED_TOTAL_NEED_DIFFERENCES,
    allocation_tolerance: float = 0.05,
    oracle_grid_size: int = 4001,
    observations_per_person: int = 100,
...
def summarize_strategy_mapping_fixed_total_need_diagnostic(
    episode_rows: Sequence[Mapping[str, object]],
) -> List[Dict[str, object]]:
...
def select_strategy_mapping_primary_environments(
    candidate_rows: Iterable[Mapping[str, object]],
    limit: int = 3,
) -> List[Mapping[str, object]]:
    """Apply the prespecified DiagnosticActiveSearch-only diagnostic-environment selection rule."""
