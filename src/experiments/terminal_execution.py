...
def audit_formal_smoke(
    *,
    ...
    if float(base_72_binding["wall_seconds"]) > 18000.0:
        raise RuntimeError("formal smoke base:72 exceeds the 18000-second runtime gate")
    ...
    if not math.isfinite(chain_seconds) or not 0.0 <= chain_seconds <= 25200.0:
        raise RuntimeError("formal smoke complete queue-excluded chain exceeds 25200 seconds")
