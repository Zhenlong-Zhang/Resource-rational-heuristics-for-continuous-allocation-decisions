"""Test purpose: validate fail-closed scheduler construction for strategy-mapping jobs."""
...
SCHEDULER_LIBRARY = PROJECT_ROOT / "scripts" / "hoffman2_scheduler.sh"


class StrategyMappingSubmissionTests(unittest.TestCase):
    def setUp(self):
        ...
        environment.update(
            {
                ...
                "FAKE_QSUB_MODE": mode,
                "STRATEGY_MAPPING_QSUB_BIN": str(self.bin_dir / "qsub"),
                "STRATEGY_MAPPING_QSTAT_BIN": str(self.bin_dir / "qstat"),
                "STRATEGY_MAPPING_QDEL_BIN": str(self.bin_dir / "qdel"),
                "STRATEGY_MAPPING_SCHEDULER_PYTHON": shutil.which("python3") or "python3",
            }
        )
    ...
    def test_valid_terse_submission_is_tracked(self):
        result = self._run(
            ...
            strategy_mapping_init_submission_tracking {str(self.evidence_dir)!r}
            strategy_mapping_submit_job family unique_valid {str(self.job_file)!r}
            [[ "$STRATEGY_MAPPING_LAST_JOB_ID" == "9001" ]]
            [[ "${{STRATEGY_MAPPING_SUBMITTED_JOB_IDS[*]}}" == "9001" ]]
            """
        )
    ...
    def test_malformed_successful_qsub_output_leaves_no_live_job(self):
        result = self._run(
            ...
            source {str(SCHEDULER_LIBRARY)!r}
            strategy_mapping_init_submission_tracking {str(self.evidence_dir)!r}
            if strategy_mapping_submit_job family unique_malformed {str(self.job_file)!r}; then
              exit 99
            fi
            """,
            mode="malformed",
    ...
    def test_later_failure_rolls_back_every_tracked_job(self):
        result = self._run(
            ...
            source {str(SCHEDULER_LIBRARY)!r}
            strategy_mapping_init_submission_tracking {str(self.evidence_dir)!r}
            strategy_mapping_submit_job first unique_first {str(self.job_file)!r}
            strategy_mapping_submit_job second unique_second {str(self.job_file)!r}
    ...
    def test_failed_qdel_and_qstat_cannot_certify_malformed_submission_cleanup(self):
        result = self._run(
            ...
            source {str(SCHEDULER_LIBRARY)!r}
            strategy_mapping_init_submission_tracking {str(self.evidence_dir)!r}
            STRATEGY_MAPPING_QDEL_BIN=/usr/bin/false
            STRATEGY_MAPPING_QSTAT_BIN=/usr/bin/false
            set +e
            strategy_mapping_submit_job family unique_unverifiable {str(self.job_file)!r}
            status=$?
            set -e
            [[ "${{status}}" -eq 70 ]]
            """,
