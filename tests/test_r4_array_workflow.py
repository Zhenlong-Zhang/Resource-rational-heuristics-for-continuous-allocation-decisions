...
from scripts.r4_array_workflow import (
    COMPLETE_MARKER,
    FAILURE_FILENAME,
    PROVENANCE_FILENAME,
    REQUIRED_OUTPUTS,
...
class R4ArrayWorkflowTest(unittest.TestCase):
    def config(self, chunks: int, max_grid_points: int | None = None) -> dict[str, object]:
        ...
        return scientific_config(
            grid="r4_diagnostic_active_search",
            grid_size=grid_size,
            chunk_count=chunks,
            preset="server",
            episodes=1200,
    ...
    def test_full_manifest_has_unique_canonical_486_task_mapping(self) -> None:
        ...
        expected = {
            0: "r4_diagnostic_active_search_chunk00_of486",
            9: "r4_diagnostic_active_search_chunk09_of486",
            99: "r4_diagnostic_active_search_chunk99_of486",
            100: "r4_diagnostic_active_search_chunk100_of486",
            485: "r4_diagnostic_active_search_chunk485_of486",
        }
        for index, shard in expected.items():
            self.assertEqual(tasks[index]["shard"], shard)
            self.assertEqual(canonical_shard_name("r4_diagnostic_active_search", index, 486), shard)
    ...
    def test_submission_script_uses_throttled_one_slot_array(self) -> None:
        script = (
            ...
            / "scripts"
            / "submit_hoffman2_round4_array.sh"
    ...
    def test_strict_collection_combines_without_running_simulations(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            with patch("scripts.r4_array_workflow.subprocess.run") as run_mock, patch(
                "scripts.generate_results.write_figures"
            ), patch(
                "scripts.r4_array_workflow.current_git_commit",
    ...
    def test_nonzero_simulation_exit_records_failed_shard(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            with patch(
                ...
                "scripts.r4_array_workflow.subprocess.run",
    ...
    def test_execution_checkout_must_match_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            ), patch("scripts.r4_array_workflow.subprocess.run") as run_mock:
    ...
    def test_progress_uses_validated_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_dir:
            manifest = self.manifest(Path(temporary_dir))
            self.populate_shard(manifest, 0)
            with patch("scripts.r4_array_workflow.query_scheduler_states", return_value={"status": "test"}):
