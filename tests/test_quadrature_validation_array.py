"""Test purpose: validate quadrature task identity, numerical evidence, and array collection."""
...
from scripts.positive_need_workflow import digest, sha256_file
from scripts.quadrature_validation_array import (
    FINAL_RESULT_NAME,
    MANIFEST_NAME,
    ORDER_PAIRS,
...
SUBMITTER = PROJECT_ROOT / "scripts" / "submit_hoffman2_quadrature_validation.sh"
...
class QuadratureValidationArrayTests(unittest.TestCase):
    ...
    @patch("scripts.quadrature_validation_array.validate_diagnostic_source_identity")
    @patch("scripts.quadrature_validation_array.diagnose_quadrature_orders")
    def test_collector_requires_90_exact_four_pair_shards(
        ...
        with TemporaryDirectory() as temporary:
            ...
            with patch(
                "scripts.quadrature_validation_array.subprocess.check_output",
    ...
    def test_submitter_has_valid_syntax_and_frozen_one_slot_array(self) -> None:
        ...
        self.assertIn("strategy_mapping_submit_job \"array\"", text)
