...
from scripts import r6_array_workflow as workflow
from src.mdp.meta_mdp import EnvironmentConfig


PROJECT_ROOT = Path(__file__).resolve().parents[1]
FROZEN_CONFIG = PROJECT_ROOT / "configs/r6_primary_environments.json"
SCHEMA_CONTRACT = PROJECT_ROOT / "configs/r6_output_schemas.json"
...
def make_manifest(
    ...
    *,
    ...
    return workflow.build_manifest(
        ...
        r4_hashes=workflow.R4_EXPECTED_HASHES,
...
class R6ArrayWorkflowTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_serious_task_map_has_exact_counts_and_ranges(self):
        ...
        with mock.patch.object(workflow, "git_is_clean", return_value=True):
            manifest = workflow.build_manifest(
                ...
                selected_rows=rows,
                selected_configs=configs,
                r4_dir=Path(self._temporary.name) / "r4",
                r4_hashes=workflow.R4_EXPECTED_HASHES,
                frozen_config_path=FROZEN_CONFIG,
                schema_contract_path=SCHEMA_CONTRACT,
                episodes=1200,
        ...
        self.assertEqual(
            ...
            {
                "r6_four_way_episodes.csv": 14400,
                "r6_four_way_policy_summary.csv": 108,
                "r6_four_way_paired_comparisons.csv": 189,
                "r6_sigma_need_episodes.csv": 3600,
                "r6_sigma_need_environment_summary.csv": 3,
                "r6_sigma_need_gap_strata.csv": 54,
                "r6_fixed_total_need_episodes.csv": 6000,
                "r6_fixed_total_need_summary.csv": 46,
            },
        )
        keys = [
            (
    ...
    def test_cli_facing_create_builds_serious_manifest(self):
        ...
        with mock.patch.object(workflow, "git_is_clean", return_value=True), mock.patch.object(
            workflow,
            "_load_frozen_inputs",
            return_value=(rows, configs, workflow.R4_EXPECTED_HASHES),
        ):
    ...
    def test_missing_or_overlapping_shard_prevents_completion(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Strict R6 collection refused"):
    ...
    def test_global_common_randomness_tamper_is_rejected(self):
        ...
        workflow.atomic_write_csv(
            rows_path,
            rows,
            manifest["expected_output_schemas"]["r6_sigma_need_episodes.csv"],
        )
    ...
    def test_manifest_source_completion_and_lock_failures(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Another R6 collector"):
            workflow.collect(manifest_path)
        lock_path.unlink()
    ...
    def test_recomputed_overlapping_task_map_and_nonfinite_rows_are_rejected(self):
        ...
        workflow.atomic_write_csv(
            rows_path,
            rows,
            manifest["expected_output_schemas"]["r6_four_way_episodes.csv"],
        )
        update_task_hash(manifest_path, int(task["task_index"]))
        with self.assertRaises(RuntimeError):
            workflow.collect(manifest_path)
