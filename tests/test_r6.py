...
from scripts.run_r6_smoke import build_parser, run as run_r6_smoke, validate_r6_artifacts
from src.experiments import r6 as r6_module
...
class R6Tests(unittest.TestCase):
    def test_four_way_uses_identical_episodes_and_time_matched_oracles(self) -> None:
        ...
        for episode_index in range(3):
            ...
            self.assertEqual(
                ...
                {R6_POLICY_RR, R6_POLICY_MANUAL, R6_POLICY_EQUAL_SPLIT, R6_POLICY_ORACLE},
            )
            for field in (
                "need_1",
                "need_2",
            ...
            rr = next(row for row in episode_rows if row["policy"] == R6_POLICY_RR)
            manual = next(row for row in episode_rows if row["policy"] == R6_POLICY_MANUAL)
            ...
            split = next(row for row in episode_rows if row["policy"] == R6_POLICY_EQUAL_SPLIT)
            oracle = next(row for row in episode_rows if row["policy"] == R6_POLICY_ORACLE)
    ...
    def test_policy_computation_seed_is_independent_of_hidden_state_seed(self) -> None:
        ...
        rr = next(row for row in rows if row["policy"] == R6_POLICY_RR)
    ...
    def test_policy_execution_order_does_not_change_scientific_rows(self) -> None:
        ...
        forward = evaluate_r6_four_way_environment(
            "order",
            config,
            episodes,
        ...
        reverse = evaluate_r6_four_way_environment(
            "order",
            config,
            episodes,
            rr_policy=MyopicValueOfInformationPolicy(observation_draws=2),
    ...
    def test_four_way_summary_is_paired_and_keeps_empty_strata(self) -> None:
        ...
        policy_summary, comparisons = summarize_r6_four_way(rows)
        all_rr = next(
            row
            for row in policy_summary
            if row["policy"] == R6_POLICY_RR and row["stratum_dimension"] == "all"
        )
        ...
        expected = sum(
            float(next(row for row in rows if row["episode_index"] == index and row["policy"] == R6_POLICY_RR)["realized_utility"])
            - float(next(row for row in rows if row["episode_index"] == index and row["policy"] == R6_POLICY_MANUAL)["realized_utility"])
            for index in range(4)
        ) / 4
    ...
    def test_recovery_classification_boundaries(self) -> None:
        ...
        r6_module._attach_recovery_classifications(policy_rows, paired)
        self.assertEqual(policy_rows[0]["r6_recovery_classification"], "successful_strategy_recovery")

    def test_sigma_builder_changes_only_sigma_need(self) -> None:
        config = smoke_config()
        built = build_r6_sigma_need_configs("anchor", config, [40.0, 10.0, 20.0])
        ...
        with self.assertRaises(ValueError):
            build_r6_sigma_need_configs("anchor", config, [10.0, 10.0])
        with self.assertRaises(ValueError):
            build_r6_sigma_need_configs("anchor", config, [0.0])
        with self.assertRaises(ValueError):
            build_r6_sigma_need_configs("anchor", config, [math.inf])
    ...
    def test_sigma_sweep_preserves_standardized_randomness(self) -> None:
        ...
        environments, strata = summarize_r6_sigma_need_sweep(
            rows,
    ...
    def test_fixed_total_need_constructs_balanced_nonnegative_states(self) -> None:
        ...
        for episode_index in range(4):
            episode_rows = [row for row in rows if row["episode_index"] == episode_index]
            self.assertEqual(len({row["observation_residual_hash_1"] for row in episode_rows}), 1)
        summaries = summarize_r6_fixed_total_need_diagnostic(rows)
    ...
    def test_primary_selection_matches_frozen_r4_schema(self) -> None:
        rows = [
            {
                "environment": name,
        ...
        selected = select_r6_primary_environments(rows)
        self.assertEqual([row["environment"] for row in selected], ["a", "b", "c"])

    def test_primary_selection_runs_on_canonical_r4_table_when_available(self) -> None:
        path = Path(
            "results/r4_diagnostic_active_search_server_20260714_array486/"
            "r4_diagnostic_manual_advantage_candidates.csv"
        )
        if not path.exists():
            self.skipTest("Canonical ignored R4 result table is unavailable")
        with path.open(newline="", encoding="utf-8") as handle:
            selected = select_r6_primary_environments(csv.DictReader(handle))
    ...
    def test_smoke_artifacts_are_deterministic_and_tampering_removes_marker(self) -> None:
        r4_dir = Path("results/r4_diagnostic_active_search_server_20260714_array486")
        if not r4_dir.exists():
            self.skipTest("Canonical ignored R4 result directory is unavailable")
        parser = build_parser()
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for output in outputs:
                ...
                validate_r6_artifacts(output)
            manifests = [
                json.loads((output / "r6_manifest.json").read_text(encoding="utf-8"))
                for output in outputs
            ]
            ...
            tampered = outputs[0] / "r6_four_way_episodes.csv"
            with tampered.open("a", encoding="utf-8") as handle:
                handle.write("tampered\n")
            with self.assertRaises(RuntimeError):
                validate_r6_artifacts(outputs[0])
            ...
            duplicate_csv = duplicate_output / "r6_four_way_episodes.csv"
            ...
            duplicate_manifest_path = duplicate_output / "r6_manifest.json"
            duplicate_manifest = json.loads(duplicate_manifest_path.read_text(encoding="utf-8"))
            duplicate_meta = duplicate_manifest["expected_outputs"]["r6_four_way_episodes.csv"]
            ...
            with self.assertRaisesRegex(RuntimeError, "Frozen expected row-count|Duplicate four-way"):
                validate_r6_artifacts(duplicate_output)
            self.assertFalse((duplicate_output / "COMPLETED.json").exists())

            nonfinite_csv = nonfinite_output / "r6_four_way_episodes.csv"
            ...
            nonfinite_manifest_path = nonfinite_output / "r6_manifest.json"
            nonfinite_manifest = json.loads(nonfinite_manifest_path.read_text(encoding="utf-8"))
            nonfinite_manifest["expected_outputs"]["r6_four_way_episodes.csv"]["sha256"] = (
            ...
            with self.assertRaisesRegex(RuntimeError, "Non-finite"):
                validate_r6_artifacts(nonfinite_output)
            self.assertFalse((nonfinite_output / "COMPLETED.json").exists())

            def rewrite_four_way(output: Path, rows: list[dict[str, str]]) -> None:
                csv_path = output / "r6_four_way_episodes.csv"
                fieldnames = list(rows[0])
                with csv_path.open("w", newline="", encoding="utf-8") as handle:
                    writer = csv.DictWriter(handle, fieldnames=fieldnames)
                    writer.writeheader()
                    writer.writerows(rows)
                manifest_path = output / "r6_manifest.json"
                manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
                metadata = manifest["expected_outputs"]["r6_four_way_episodes.csv"]
            ...
            original_csv = outputs[1] / "r6_four_way_episodes.csv"
            ...
            with self.assertRaisesRegex(RuntimeError, "Frozen expected row-count|Missing or extra"):
                validate_r6_artifacts(missing_output)
            ...
            with self.assertRaisesRegex(RuntimeError, "Wrong policy|Missing or extra"):
                validate_r6_artifacts(wrong_policy_output)
            ...
            with self.assertRaisesRegex(RuntimeError, "common-randomness mismatch"):
                validate_r6_artifacts(randomness_output)
            ...
            with self.assertRaisesRegex(RuntimeError, "stale manifest hash"):
                validate_r6_artifacts(stale_completion_output)
            ...
            with self.assertRaisesRegex(RuntimeError, "cannot claim scientific completion"):
                validate_r6_artifacts(invalid_completion_output)
            self.assertFalse(completion_path.exists())

            oracle_nan_output = root / "oracle_nan"
            shutil.copytree(outputs[1], oracle_nan_output)
            sigma_path = oracle_nan_output / "r6_sigma_need_episodes.csv"
            ...
            manifest_path = oracle_nan_output / "r6_manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["expected_outputs"]["r6_sigma_need_episodes.csv"]["sha256"] = (
            ...
            with self.assertRaisesRegex(RuntimeError, "Non-finite"):
                validate_r6_artifacts(oracle_nan_output)
