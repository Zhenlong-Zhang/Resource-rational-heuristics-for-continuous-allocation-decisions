"""Test purpose: validate positive-need environments, finite-support episodes, and policy-comparison semantics."""
...
class PositiveNeedModelTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    @patch(
        "src.experiments.positive_need._dense_sample_action_value"
    )
    @patch(
        "src.experiments.positive_need.FiniteSupportMetaMDP.solve_terminal_allocation",
        return_value=(0.5, 1.0),
    )
    @patch(
        "src.experiments.positive_need.FiniteSupportMyopicVOIPolicy.choose_action",
        return_value="sample_1",
    )
    @patch(
        "src.experiments.positive_need.FiniteSupportMyopicVOIPolicy.action_values",
        autospec=True,
    )
