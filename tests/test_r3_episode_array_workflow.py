...
from scripts.r3_episode_array_workflow import (
    COMPLETION_MARKER,
    EPISODE_FILENAME,
    ExclusiveLock,
    MANIFEST_FILENAME,
...
class R3EpisodeArrayWorkflowTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _write_writer_evidence(self, final_qstat_text: str = "header\nheader\n") -> None:
        ...
        payload = {
            ...
            "writer_job_name_pattern": "rr_r3_m",
    ...
    def test_strict_stage_preserves_frozen_rows_and_runs_global_combiner_on_stage(self) -> None:
        ...
        with mock.patch(
            "scripts.r3_episode_array_workflow.run_single_episode_from_task_metadata",
    ...
    def test_production_settings_and_dirty_checkout_are_rejected(self) -> None:
        ...
        with mock.patch(
            "scripts.r3_episode_array_workflow.tracked_checkout_is_clean",
            return_value=False,
        ):
    ...
    def test_final_qstat_and_qacct_hashes_are_revalidated(self) -> None:
        self._write_writer_evidence(
            "header\nheader\nold-array user rr_r3_m r queue\n"
        )
        ...
        child = subprocess.run(
            [
                ...
                (
                    ...
                    "from scripts.r3_episode_array_workflow import ExclusiveLock; "
    ...
    def _run_submit_script(self, manifest: dict[str, object], fail_call: int = 0):
        ...
        completed = subprocess.run(
            ["bash", "scripts/submit_hoffman2_round3_episode_array.sh"],
            cwd=Path(__file__).parents[1],
            env=env,
            capture_output=True,
            text=True,
    ...
    def test_fake_qsub_generates_and_executes_deferred_lane_job(self) -> None:
        ...
        lane_job = job_dir / "r3_episode_array_lane_1.job"
    ...
    def test_lane_submission_failure_rolls_back_recorded_job(self) -> None:
        ...
        journal = json.loads(
            (self.workflow / "r3_episode_array_submission.json").read_text(encoding="utf-8")
        )
    ...
    def test_submission_scripts_use_disjoint_one_slot_lanes(self) -> None:
        submit = (Path(__file__).parents[1] / "scripts" / "submit_hoffman2_round3_episode_array.sh").read_text(
            encoding="utf-8"
        )
        cutover = (Path(__file__).parents[1] / "scripts" / "prepare_hoffman2_round3_episode_cutover.sh").read_text(
            encoding="utf-8"
        )
