...
# Submit method comparison Hoffman2 jobs.
#
# Defaults:
...
ACTIVE_OUTPUT_DIR="${ACTIVE_OUTPUT_DIR:-results/method_comparison_active_search_equal_outcome_server}"

METHOD_H_RT="${METHOD_H_RT:-24:00:00}"
METHOD_OUTPUT_DIR="${METHOD_OUTPUT_DIR:-results/method_comparison_10x_server}"

CHECK_H_RT="${CHECK_H_RT:-02:00:00}"
CHECK_OUTPUT_JSON="${CHECK_OUTPUT_JSON:-results/method_comparison_observation_stream_check_server.json}"
...
if [[ "${RUN_OBSERVATION_CHECK}" == "1" ]]; then
  job_file="${PROJECT_ROOT}/jobs/method_comparison_observation_stream_check.job"
  cat > "${job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_observation_stream_check.\$JOB_ID.log
#$ -j y
#$ -l h_rt=${CHECK_H_RT},h_data=${H_DATA}
#$ -N rr_method_comparison_obs
...
if [[ "${RUN_ACTIVE_SEARCH}" == "1" ]]; then
  job_file="${PROJECT_ROOT}/jobs/method_comparison_active_search_equal_outcome_server.job"
  cat > "${job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_active_search_equal_outcome_server.\$JOB_ID.log
#$ -j y
#$ -pe shared ${SLOTS}
#$ -l h_rt=${ACTIVE_H_RT},h_data=${H_DATA}
#$ -N rr_method_comparison_active
...
if [[ "${RUN_METHOD_COMPARISON_10X}" == "1" ]]; then
  job_file="${PROJECT_ROOT}/jobs/method_comparison_10x_server.job"
  cat > "${job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_10x_server.\$JOB_ID.log
#$ -j y
#$ -pe shared ${SLOTS}
...
echo
echo "Submitted requested method comparison Hoffman2 jobs. Check status with:"
echo "  qstat -u \${USER}"
echo
