#!/usr/bin/env python3
"""Build a validated, professor-facing Round 5 report from collected results."""
...
def short_environment(value: str) -> str:
    if value.startswith("confirmation_"):
        return value.split("_r5_six", 1)[0].replace("confirmation_", "Candidate ")
    return value
...
<title>Resource-Rational Allocation: Round 5 Results</title>
<style>
...
<header><h1>Resource-Rational Allocation</h1><p class="muted">Round 5: objective diagnosis, information value, independent active-search confirmation, and a frozen non-myopic solver check.</p></header>
...
<img src="figures/r5_fixed_budget_utility.svg" alt="Utility by fixed observation budget">
...
<img src="figures/r5_confirmation_rates.svg" alt="Independent confirmation rates and one-sided lower bounds">
...
<img src="figures/r5_solver_paired_utility.svg" alt="Paired DP minus myopic utility differences">
<h2>Conclusion</h2>
...
<h2>Supporting Data</h2><p><a href="supporting_data/r5_confirmation_comparison.csv">Confirmation table</a> · <a href="supporting_data/r5_joint_candidates_discovery.csv">Discovery candidates</a> · <a href="supporting_data/r5_solver_comparison.csv">Solver comparison</a> · <a href="supporting_data/r5_report_summary.json">Report summary</a></p>
...
def build_report(args: argparse.Namespace) -> dict[str, object]:
    ...
    validate_confirmation_manifests(
        load_json(args.discovery_dir / "r5_manifest.json"),
        load_json(args.confirmation_dir / "r5_manifest.json"),
    )

    oracle_analysis = load_json(args.oracle_analysis_dir / "r5_oracle_analysis.json")
    strata = oracle_strata(
        read_csv(args.oracle_analysis_dir / "r5_oracle_sign_summary.csv")
    )
    fixed_rows = read_csv(args.formal_dir / "r5_fixed_budget_evidence.csv")
    ...
    discovery_rows = read_csv(args.discovery_dir / "r5_joint_candidates.csv")
    ...
    confirmation_rows = read_csv(
        args.confirmation_dir / "r5_rr_environment_summary.csv"
    )
    solver_rows = read_csv(args.solver_dir / "r5_solver_comparison.csv")
    ...
    write_confirmation_svg(figures / "r5_confirmation_rates.svg", confirmation_rows)
    write_csv(supporting / "r5_confirmation_comparison.csv", table)
    copy_if_present(
        args.formal_dir / "figures" / "r5_fixed_budget_utility.svg",
        figures / "r5_fixed_budget_utility.svg",
    )
    copy_if_present(
        args.solver_dir / "figures" / "r5_solver_paired_utility.svg",
        figures / "r5_solver_paired_utility.svg",
    )
    for source, destination in (
        (
            args.discovery_dir / "r5_joint_candidates.csv",
            supporting / "r5_joint_candidates_discovery.csv",
        ),
        (
            args.confirmation_dir / "r5_rr_environment_summary.csv",
            supporting / "r5_confirmation_environment_summary.csv",
        ),
        (
            args.solver_dir / "r5_solver_comparison.csv",
            supporting / "r5_solver_comparison.csv",
        ),
        (
            args.oracle_analysis_dir / "r5_oracle_sign_summary.csv",
            supporting / "r5_oracle_sign_summary.csv",
        ),
        (
            args.formal_dir / "r5_fixed_budget_evidence.csv",
            supporting / "r5_fixed_budget_evidence.csv",
        ),
        (args.oracle_dir / "COMPLETED.json", supporting / "r5_oracle_completed.json"),
        (
            args.discovery_dir / "COMPLETED.json",
            supporting / "r5_discovery_completed.json",
        ),
        (
            args.confirmation_dir / "COMPLETED.json",
            supporting / "r5_confirmation_completed.json",
        ),
    ):
        copy_if_present(source, destination)
    (supporting / "r5_report_summary.json").write_text(
    ...
    copy_if_present(
        PROJECT_ROOT / "notebooks" / "run_round5_pipeline.ipynb",
        reproducibility / "run_round5_pipeline.ipynb",
    )
    (args.output_dir / "README.md").write_text(
        ...
        "- Notebook: `reproducibility/run_round5_pipeline.ipynb`\n"
