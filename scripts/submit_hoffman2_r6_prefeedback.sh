...
# Submit one R6 pre-feedback development or confirmation array.
...
case "${PHASE}" in
  development)
    ...
    OUTPUT_DIR="${OUTPUT_DIR:-results/r6_prefeedback_development_$(date +%Y%m%d_%H%M%S)}"
    ;;
  smoke)
    ...
    OUTPUT_DIR="${OUTPUT_DIR:-results/r6_prefeedback_smoke_$(date +%Y%m%d_%H%M%S)}"
    ;;
  serious)
    : "${THROTTLE:?Set THROTTLE from the current Hoffman2 Compute Ceiling Report}"
    : "${DEVELOPMENT_DIR:?Set DEVELOPMENT_DIR to the validated development output}"
    COLLECT_H_RT="${COLLECT_H_RT:-04:00:00}"
    OUTPUT_DIR="${OUTPUT_DIR:-results/r6_prefeedback_serious_$(date +%Y%m%d_%H%M%S)}"
    ;;
  *)
...
if [[ "${PHASE}" == "development" ]]; then
  "${PYTHON_BIN}" scripts/r6_prefeedback_workflow.py create-development \
  ...
  "${PYTHON_BIN}" scripts/r6_prefeedback_workflow.py create-confirmation \
    --output-dir "${OUTPUT_DIR_ABS}" \
    --development-dir "${DEVELOPMENT_DIR_ABS}" \
    --mode "${PHASE}"
fi

MANIFEST="${OUTPUT_DIR_ABS}/r6_prefeedback_manifest.json"
...
PHASE_TAG="${PHASE:0:1}"
ARRAY_JOB="${SCHEDULER_DIR}/r6_prefeedback_${PHASE}_array.job"
cat > "${ARRAY_JOB}" <<EOF
#!/usr/bin/env bash
#$ -cwd
#$ -N r6p${PHASE_TAG}
#$ -q ${QUEUE}
#$ -j y
#$ -o ${LOG_DIR}/r6_prefeedback_${PHASE}.$JOB_ID.$TASK_ID.log
#$ -l h_rt=${TASK_H_RT}
...
export LANG=C
cd "${PROJECT_ROOT}"
"${PYTHON_BIN}" scripts/r6_prefeedback_workflow.py run-task \
  --manifest "${MANIFEST}" \
...
ARRAY_JOB_NAME="r6p${PHASE_TAG}a${RUN_TOKEN}"
r6_submit_job "array" "${ARRAY_JOB_NAME}" "${ARRAY_JOB}"
ARRAY_JOB_ID="${R6_LAST_JOB_ID}"

COLLECT_JOB="${SCHEDULER_DIR}/r6_prefeedback_${PHASE}_collect.job"
cat > "${COLLECT_JOB}" <<EOF
#!/usr/bin/env bash
#$ -cwd
#$ -N r6p${PHASE_TAG}c
#$ -q ${QUEUE}
#$ -j y
#$ -o ${LOG_DIR}/r6_prefeedback_${PHASE}_collect.$JOB_ID.log
#$ -l h_rt=${COLLECT_H_RT}
...
export LANG=C
cd "${PROJECT_ROOT}"
"${PYTHON_BIN}" scripts/r6_prefeedback_workflow.py collect --manifest "${MANIFEST}"
EOF
chmod +x "${COLLECT_JOB}"
COLLECT_JOB_NAME="r6p${PHASE_TAG}c${RUN_TOKEN}"
...
printf 'progress: %s scripts/r6_prefeedback_workflow.py progress --manifest %s\n' \
  "${PYTHON_BIN}" "${MANIFEST}"
