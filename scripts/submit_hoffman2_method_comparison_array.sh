...
# Submit the method comparison 1200-episode approximation-method comparison as a
...
OUTPUT_DIR="${OUTPUT_DIR:-results/method_comparison_approximation_methods_checkpointed_1200ep}"
...
env_file="${PROJECT_ROOT}/jobs/method_comparison_approx_methods_envs.txt"
split_csv "${ENVIRONMENTS}" > "${env_file}"

task_file="${PROJECT_ROOT}/jobs/method_comparison_approx_methods_tasks.tsv"
: > "${task_file}"
...
array_job_file="${PROJECT_ROOT}/jobs/method_comparison_approximation_methods_1200ep_array.job"
cat > "${array_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_approximation_methods_1200ep_array.\$JOB_ID.\$TASK_ID.log
#$ -j y
#$ -l h_rt=${TASK_H_RT},h_data=${TASK_H_DATA}
#$ -N rr_method_comparison_marr
...
PYTHON=${PYTHON_BIN}
TASK_LINE=\$(sed -n "\${SGE_TASK_ID}p" jobs/method_comparison_approx_methods_tasks.tsv)
if [[ -z "\${TASK_LINE}" ]]; then
...
combine_job_file="${PROJECT_ROOT}/jobs/method_comparison_approximation_methods_1200ep_array_combine.job"
cat > "${combine_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_approximation_methods_1200ep_array_combine.\$JOB_ID.log
#$ -j y
#$ -l h_rt=${COMBINE_H_RT},h_data=${COMBINE_H_DATA}
#$ -N rr_method_comparison_mcomb
...
\$PYTHON scripts/combine_method_comparison_results.py \\
  ...
  --task-manifest jobs/method_comparison_approx_methods_tasks.tsv \\
  --require-complete

tar -czf "${OUTPUT_DIR}.tar.gz" "${OUTPUT_DIR}" \\
  jobs/method_comparison_approx_methods_envs.txt \\
  jobs/method_comparison_approx_methods_tasks.tsv \\
  "${OUTPUT_DIR}/run_config.txt" \\
  logs/method_comparison_approximation_methods_1200ep_array.*.log \\
  logs/method_comparison_approximation_methods_1200ep_array_combine.*.log
