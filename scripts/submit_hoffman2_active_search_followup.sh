...
# Submit method comparison follow-up Hoffman2 jobs.
#
...
ACTIVE_OUTPUT_DIR="${ACTIVE_OUTPUT_DIR:-results/method_comparison_active_search_equal_outcome_narrow_followup_1200ep}"
...
APPROX_OUTPUT_DIR="${APPROX_OUTPUT_DIR:-results/method_comparison_approximation_methods_1200ep}"
...
echo "method comparison follow-up settings"
echo "  grid: ${GRID}"
...
if [[ "${SUBMIT_ACTIVE}" == "1" ]]; then
  ...
  array_job_file="${PROJECT_ROOT}/jobs/method_comparison_active_search_narrow_followup_array.job"
  cat > "${array_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_active_search_narrow_followup.\$JOB_ID.\$TASK_ID.log
#$ -j y
#$ -t 1-${grid_tasks}
#$ -tc ${MAX_CONCURRENT_TASKS}
#$ -l h_rt=${TASK_H_RT},h_data=${TASK_H_DATA}
#$ -N rr_method_comparison_nf_arr
...
EOF
  ...
  combine_job_file="${PROJECT_ROOT}/jobs/method_comparison_active_search_narrow_followup_combine.job"
  cat > "${combine_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_active_search_narrow_followup_combine.\$JOB_ID.log
#$ -j y
#$ -pe shared ${COMBINE_SLOTS}
#$ -l h_rt=${COMBINE_H_RT},h_data=${COMBINE_H_DATA}
#$ -N rr_method_comparison_nf_comb
...
EOF
  ...
  package_job_file="${PROJECT_ROOT}/jobs/method_comparison_active_search_narrow_followup_package.job"
  cat > "${package_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_active_search_narrow_followup_package.\$JOB_ID.log
#$ -j y
#$ -l h_rt=${PACKAGE_H_RT},h_data=${PACKAGE_H_DATA}
#$ -N rr_method_comparison_nf_pkg
...
tar -czf "${ACTIVE_OUTPUT_DIR}.tar.gz" "${ACTIVE_OUTPUT_DIR}" \\
  logs/method_comparison_active_search_narrow_followup.*.log \\
  logs/method_comparison_active_search_narrow_followup_combine.*.log
...
if [[ "${SUBMIT_APPROX}" == "1" ]]; then
  ...
  approx_job_file="${PROJECT_ROOT}/jobs/method_comparison_approximation_methods_1200ep.job"
  cat > "${approx_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_approximation_methods_1200ep.\$JOB_ID.log
#$ -j y
#$ -pe shared ${APPROX_SLOTS}
#$ -l h_rt=${APPROX_H_RT},h_data=${APPROX_H_DATA}
#$ -N rr_method_comparison_methods
...
tar -czf "${APPROX_OUTPUT_DIR}.tar.gz" "${APPROX_OUTPUT_DIR}" \\
  logs/method_comparison_approximation_methods_1200ep.*.log
...
echo
echo "Submitted requested method comparison follow-up jobs."
