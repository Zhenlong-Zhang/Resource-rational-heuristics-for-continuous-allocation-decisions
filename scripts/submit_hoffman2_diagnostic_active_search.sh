...
# Submit the Active-search diagnostic grid as independent one-slot Hoffman2 array tasks.
...
OUTPUT_DIR="${OUTPUT_DIR:-results/active_search_benchmark_server}"
GRID="${GRID:-active_search_benchmark}"
...
if ! git diff --quiet "${BASELINE_COMMIT}" -- \
  ...
  src/solvers \
  src/simulator; then
  echo "Scientific source differs from DiagnosticActiveSearch baseline ${BASELINE_COMMIT}; refusing submission." >&2
  exit 1
fi
...
if [[ "${GRID_TASKS}" -le 0 ]]; then
  echo "No DiagnosticActiveSearch grid tasks were generated." >&2
  exit 1
fi

manifest_command=(
  "${PYTHON_BIN}" scripts/diagnostic_active_search_workflow.py create-manifest
  --run-dir "${OUTPUT_DIR}"
...
MANIFEST="${PROJECT_ROOT}/${OUTPUT_DIR}/diagnostic_active_search_array_manifest.json"
ARRAY_JOB_FILE="${PROJECT_ROOT}/jobs/active_search_benchmark_array.job"
cat > "${ARRAY_JOB_FILE}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/active_search_benchmark_array.\$JOB_ID.\$TASK_ID.log
#$ -j y
#$ -t 1-${GRID_TASKS}
#$ -tc ${MAX_CONCURRENT_TASKS}
#$ -l h_rt=${TASK_H_RT},h_data=${TASK_H_DATA}
#$ -N rr_diagnostic_active_search_arr
...
${PYTHON_BIN} scripts/diagnostic_active_search_workflow.py run-shard \\
  --manifest "${MANIFEST}" \\
...
if [[ -z "${array_job_id}" ]]; then
  echo "Could not parse DiagnosticActiveSearch array job id." >&2
  exit 1
fi

COLLECT_JOB_FILE="${PROJECT_ROOT}/jobs/active_search_benchmark_collect.job"
cat > "${COLLECT_JOB_FILE}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/active_search_benchmark_collect.\$JOB_ID.log
#$ -j y
#$ -l h_rt=${COLLECT_H_RT},h_data=${COLLECT_H_DATA}
#$ -N rr_diagnostic_active_search_col

export LC_ALL=C
export LANG=C
set -euo pipefail

${PYTHON_BIN} scripts/diagnostic_active_search_workflow.py collect --manifest "${MANIFEST}"
tar -czf "${OUTPUT_DIR}.tar.gz" "${OUTPUT_DIR}" \\
  logs/active_search_benchmark_array.${array_job_id}.*.log \\
  logs/active_search_benchmark_collect.\$JOB_ID.log
...
if [[ -z "${collector_job_id}" ]]; then
  echo "Could not parse DiagnosticActiveSearch collector job id." >&2
  exit 1
fi

"${PYTHON_BIN}" scripts/diagnostic_active_search_workflow.py record-jobs \
  --manifest "${MANIFEST}" \
  --array-job-id "${array_job_id}" \
  --collector-job-id "${collector_job_id}"

echo "DiagnosticActiveSearch array job: ${array_job_id}"
echo "DiagnosticActiveSearch collector job: ${collector_job_id}"
echo "DiagnosticActiveSearch tasks: ${GRID_TASKS}; throttle: ${MAX_CONCURRENT_TASKS}"
echo "Progress command:"
echo "  ${PYTHON_BIN} scripts/diagnostic_active_search_workflow.py progress --manifest ${MANIFEST}"
