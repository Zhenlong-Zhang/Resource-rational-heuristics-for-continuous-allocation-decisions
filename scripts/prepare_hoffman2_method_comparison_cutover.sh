...
CANONICAL_OUTPUT_DIR="${CANONICAL_OUTPUT_DIR:-results/method_comparison_approximation_methods_checkpointed_1200ep_20260713}"
...
TASK_MANIFEST="${TASK_MANIFEST:-jobs/method_comparison_approx_methods_tasks.tsv}"
SMOKE_GATE="${SMOKE_GATE:-}"
REVIEWED_COMMIT="${REVIEWED_COMMIT:-}"
WRITER_JOB_IDS="${WRITER_JOB_IDS:-14055056,14055057}"
SUCCESSOR_JOB_IDS="${SUCCESSOR_JOB_IDS:-}"
WRITER_JOB_NAME_PATTERN="${WRITER_JOB_NAME_PATTERN:-rr_method_comparison_m}"
...
"${PYTHON_BIN}" scripts/method_comparison_episode_workflow.py verify-smoke-gate \
...
if [[ -n "${unaccounted_ids}" ]]; then
  echo "Unaccounted possible MethodComparison writer jobs: ${unaccounted_ids}" >&2
  exit 3
fi
...
if [[ -n "${final_unaccounted_ids}" ]]; then
  echo "Final qstat found a known or successor MethodComparison writer: ${final_unaccounted_ids}" >&2
  exit 6
fi
...
freeze_command=(
  "${PYTHON_BIN}" scripts/method_comparison_episode_workflow.py freeze
