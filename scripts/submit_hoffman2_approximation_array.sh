...
# Submit the method comparison active-search grid as a Hoffman2 array job.
#
# Why this exists:
...
OUTPUT_DIR="${OUTPUT_DIR:-results/method_comparison_active_search_equal_outcome_server_array486}"
...
array_job_file="${PROJECT_ROOT}/jobs/method_comparison_active_search_array486.job"
cat > "${array_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_active_search_array486.\$JOB_ID.\$TASK_ID.log
#$ -j y
#$ -t 1-${ARRAY_TASKS}
#$ -tc ${MAX_CONCURRENT_TASKS}
#$ -l h_rt=${TASK_H_RT},h_data=${TASK_H_DATA}
#$ -N rr_method_comparison_arr
...
combine_job_file="${PROJECT_ROOT}/jobs/method_comparison_active_search_array486_combine.job"
cat > "${combine_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_active_search_array486_combine.\$JOB_ID.log
#$ -j y
#$ -pe shared ${COMBINE_SLOTS}
#$ -l h_rt=${COMBINE_H_RT},h_data=${COMBINE_H_DATA}
#$ -N rr_method_comparison_comb
...
package_job_file="${PROJECT_ROOT}/jobs/method_comparison_active_search_array486_package.job"
cat > "${package_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o logs/method_comparison_active_search_array486_package.\$JOB_ID.log
#$ -j y
#$ -l h_rt=${PACKAGE_H_RT},h_data=${PACKAGE_H_DATA}
#$ -N rr_method_comparison_pkg
...
tar -czf "${ARCHIVE}" "${OUTPUT_DIR}" \\
  results/method_comparison_observation_stream_check_server.json \\
  logs/method_comparison_active_search_array486.*.log \\
  logs/method_comparison_active_search_array486_combine.*.log
ls -lh "${ARCHIVE}"
date
EOF
...
echo
echo "Submitted method comparison array workflow:"
