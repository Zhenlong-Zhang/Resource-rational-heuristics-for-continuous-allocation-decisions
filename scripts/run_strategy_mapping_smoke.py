...
DIAGNOSTIC_ACTIVE_SEARCH_EXPECTED_HASHES = {
    "active_search_diagnostic_environment_summary.csv": "84bb0e5efede45b4453cdfffca037a299a3dc9e212807a236579df5030f3f3a0",
    "active_search_diagnostic_manual_advantage_candidates.csv": "c668d43b7538481c88983ea42dde3067e5a96182f76d0e689616524df4810ddd",
    "diagnostic_active_search_array_manifest.json": "7e4e86c40e0656f7e42ee168ccf59587e61d7a56f6ad77ba9810a333c4949aeb",
}
...
def validate_strategy_mapping_artifacts(output_dir: Path) -> dict[str, object]:
    ...
    manifest_path = output_dir / "strategy_mapping_manifest.json"
    if not manifest_path.exists():
        raise RuntimeError("Missing strategy_mapping_manifest.json")
    ...
    for filename, expected in manifest["expected_outputs"].items():
        ...
        if not path.exists():
            raise RuntimeError(f"Missing required StrategyMapping output: {filename}")
    ...
    four_way = tables["strategy_mapping_four_way_episodes.csv"]
    _require_finite(
        four_way,
        (
            "episode_index",
        ...
        "strategy_mapping_four_way_episodes",
    )
    ...
    sigma_rows = tables["strategy_mapping_sigma_need_episodes.csv"]
    _require_finite(
        sigma_rows,
        (
            "episode_index",
        ...
        "strategy_mapping_sigma_need_episodes",
    )
    ...
    fixed_rows = tables["strategy_mapping_fixed_total_need_episodes.csv"]
    _require_finite(
        fixed_rows,
        (
            "episode_index",
        ...
        "strategy_mapping_fixed_total_need_episodes",
    )
    fixed_keys = [
    ...
    if previous_completion is not None:
        ...
        validation_path = output_dir / "strategy_mapping_validation.json"
...
def _load_selected_configs(diagnostic_active_search_dir: Path, frozen_config_path: Path):
    observed_hashes = {}
    ...
    if [row["environment"] for row in frozen_rows] != [row["environment"] for row in selected_rows]:
        raise RuntimeError("Frozen StrategyMapping configuration order does not match deterministic DiagnosticActiveSearch selection")
    candidate_fields = (
        "mu_need",
        "sigma_need",
        "sigma_sample",
    ...
    for source, frozen_row in zip(selected_rows, frozen_rows):
        if int(float(source["grid_index"])) != int(frozen_row["source_grid_index"]):
            raise RuntimeError("Frozen StrategyMapping grid index does not match the DiagnosticActiveSearch source row")
        for field in candidate_fields:
            if float(source[field]) != float(frozen_row["config"][field]):
                raise RuntimeError(f"Frozen StrategyMapping parameter mismatch for {field}")
    selected_configs = [
        (
            str(row["environment"]),
            replace(
...
def run(args: argparse.Namespace) -> None:
    ...
    selected_rows, selected_configs, observed_hashes, frozen_config_hash = _load_selected_configs(
        args.diagnostic_active_search_dir.resolve(), args.frozen_config.resolve()
    )
    ...
    for position, (environment, config) in enumerate(selected_configs, start=1):
        print(f"[StrategyMapping smoke] four-way environment {position}/{len(selected_configs)}: {environment}")
        episodes = [
    ...
    policy_summary, paired_summary = summarize_strategy_mapping_four_way(four_way_rows)

    primary_environment, primary_config = selected_configs[0]
    print(f"[StrategyMapping smoke] controlled sigma_need sweep around {primary_environment}")
    sigma_rows = evaluate_strategy_mapping_sigma_need_sweep(
        primary_environment,
        primary_config,
    ...
    sigma_summary, sigma_gap_summary = summarize_strategy_mapping_sigma_need_sweep(sigma_rows)

    print(f"[StrategyMapping smoke] fixed-total-need diagnostic around {primary_environment}")
    fixed_rows = evaluate_strategy_mapping_fixed_total_need_diagnostic(
        primary_environment,
        primary_config,
    ...
    fixed_summary = summarize_strategy_mapping_fixed_total_need_diagnostic(fixed_rows)

    tables = {
        "strategy_mapping_four_way_episodes.csv": four_way_rows,
        "strategy_mapping_four_way_policy_summary.csv": policy_summary,
        "strategy_mapping_four_way_paired_comparisons.csv": paired_summary,
        "strategy_mapping_sigma_need_episodes.csv": sigma_rows,
        "strategy_mapping_sigma_need_environment_summary.csv": sigma_summary,
        "strategy_mapping_sigma_need_gap_strata.csv": sigma_gap_summary,
        "strategy_mapping_fixed_total_need_episodes.csv": fixed_rows,
    ...
    sigma_summary_strata = 2 + 2 + 4 + (len(STRATEGY_MAPPING_DEFAULT_GAP_BIN_EDGES) - 1) + (
        len(STRATEGY_MAPPING_DEFAULT_TOTAL_NEED_BIN_EDGES) - 1
    )
    fixed_summary_strata = 1 + 2 + 2 + 4
    expected_row_counts = {
        "strategy_mapping_four_way_episodes.csv": len(selected_configs) * args.n_episodes * 4,
        "strategy_mapping_four_way_policy_summary.csv": len(selected_configs) * four_way_strata * 4,
        "strategy_mapping_four_way_paired_comparisons.csv": len(selected_configs) * four_way_strata * 7,
        "strategy_mapping_sigma_need_episodes.csv": len(args.sigma_need_values) * args.n_episodes,
        "strategy_mapping_sigma_need_environment_summary.csv": len(args.sigma_need_values),
        "strategy_mapping_sigma_need_gap_strata.csv": len(args.sigma_need_values) * sigma_summary_strata,
        "strategy_mapping_fixed_total_need_episodes.csv": len(STRATEGY_MAPPING_FIXED_TOTAL_NEED_DIFFERENCES)
        * args.fixed_total_episodes,
        "strategy_mapping_fixed_total_need_summary.csv": len(STRATEGY_MAPPING_FIXED_TOTAL_NEED_DIFFERENCES)
    ...
    source_files = (
        PROJECT_ROOT / "src/experiments/strategy_mapping.py",
        PROJECT_ROOT / "scripts/run_strategy_mapping_smoke.py",
        PROJECT_ROOT / "tests/test_strategy_mapping.py",
        args.frozen_config.resolve(),
    )
    manifest = {
        "schema_version": 1,
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "workflow": "strategy_mapping_pre_feedback_smoke",
        "analysis_mode": "smoke",
        ...
        "diagnostic_active_search_input_dir": str(args.diagnostic_active_search_dir.resolve()),
        "diagnostic_active_search_input_hashes": observed_hashes,
    ...
    _atomic_write_json(output_dir / "strategy_mapping_manifest.json", manifest)

    validation = validate_strategy_mapping_artifacts(output_dir)
    _atomic_write_json(output_dir / "strategy_mapping_validation.json", validation)
    completion = {
        "schema_version": 1,
        "status": "strict_smoke_validation_complete",
        "scientific_completion": False,
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "manifest_sha256": _sha256(output_dir / "strategy_mapping_manifest.json"),
        "validation_sha256": _sha256(output_dir / "strategy_mapping_validation.json"),
    }
    _atomic_write_json(completion_path, completion)
    print(f"[StrategyMapping smoke] completed: {output_dir}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the prespecified strategy mapping smoke analysis.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--frozen-config",
        type=Path,
        default=Path("configs/strategy_mapping_environments.json"),
    )
    parser.add_argument("--output-dir", type=Path, default=Path("results/strategy_mapping_pre_feedback_smoke"))
