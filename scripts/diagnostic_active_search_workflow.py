...
REQUIRED_OUTPUTS = (
    "active_search_diagnostic_policy_profiles.csv",
...
MANIFEST_FILENAME = "diagnostic_active_search_array_manifest.json"
PROGRESS_FILENAME = "diagnostic_active_search_array_progress.json"
SCHEDULER_FILENAME = "diagnostic_active_search_scheduler_jobs.json"
FAILURE_FILENAME = "task_failure.json"
COMPLETE_MARKER = "DIAGNOSTIC_ACTIVE_SEARCH_ARRAY_COMPLETE"
...
def build_manifest(
    *,
    ...
    for chunk_index in range(chunk_count):
        ...
        output_dir = run_dir / "tasks" / "active_search_diagnostic" / shard
    ...
    return {
        "schema_version": 1,
        "workflow": "active_search_benchmark_array",
        "run_dir": str(run_dir),
...
def validate_manifest_structure(manifest: Mapping[str, object]) -> List[str]:
    ...
    if manifest.get("workflow") != "active_search_benchmark_array":
...
def validate_task_output(
    ...
    summary_rows = read_csv_rows(output_dir / "active_search_diagnostic_environment_summary.csv")
    try:
...
def combine_validated_outputs(manifest: Mapping[str, object]) -> None:
    ...
    if not result.ok:
        raise RuntimeError(
            "Strict DiagnosticActiveSearch collection refused: "
    ...
    result_sets = {
        "active_search_diagnostic_policy_profiles": rows_by_file["active_search_diagnostic_policy_profiles.csv"],
        "active_search_diagnostic_environment_summary": rows_by_file["active_search_diagnostic_environment_summary.csv"],
        "active_search_diagnostic_manual_advantage_candidates": rows_by_file[
            "active_search_diagnostic_manual_advantage_candidates.csv"
        ],
    }
...
def run_manifest_shard(manifest: Mapping[str, object], chunk_index: int) -> int:
    structure_errors = validate_manifest_structure(manifest)
    if structure_errors:
        raise RuntimeError(f"Invalid DiagnosticActiveSearch manifest: {structure_errors}")
    require_execution_checkout(manifest)
    task = manifest_task(manifest, chunk_index)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manifest, provenance, progress, and strict collection for DiagnosticActiveSearch arrays.")
    ...
    create.add_argument("--grid", default="active_search_benchmark")
