#!/usr/bin/env python3
"""Create reproducible approximation and active-search analysis tables and a professor-facing report."""
...
METHOD_COMPARISON_SUMMARY = "rr_approximation_methods_comparison.csv"
METHOD_COMPARISON_EPISODES = "rr_approximation_method_episode_results.csv"
DIAGNOSTIC_ACTIVE_SEARCH_SUMMARY = "active_search_diagnostic_environment_summary.csv"
DIAGNOSTIC_ACTIVE_SEARCH_PROFILES = "active_search_diagnostic_policy_profiles.csv"
DIAGNOSTIC_ACTIVE_SEARCH_CANDIDATES = "active_search_diagnostic_manual_advantage_candidates.csv"
...
def canonical_method_comparison_policies(summary_rows: Sequence[Mapping[str, str]]) -> dict[str, dict[str, str]]:
...
def analyze_method_comparison(method_comparison_dir: Path, output_dir: Path) -> dict[str, object]:
    _, summaries = read_rows(method_comparison_dir / METHOD_COMPARISON_SUMMARY)
    policies = canonical_method_comparison_policies(summaries)
    ...
    with (method_comparison_dir / METHOD_COMPARISON_EPISODES).open("r", encoding="utf-8", newline="") as handle:
    ...
    write_rows(output_dir / "method_comparison_paired_method_comparisons.csv", paired_fields, paired_rows)
    ...
    write_rows(
        output_dir / "method_comparison_dp_sensitivity.csv",
...
def analyze_active_search_diagnostic(
    diagnostic_active_search_dir: Path,
    output_dir: Path,
) -> dict[str, object]:
    _, rows = read_rows(diagnostic_active_search_dir / DIAGNOSTIC_ACTIVE_SEARCH_SUMMARY)
    _, profiles = read_rows(diagnostic_active_search_dir / DIAGNOSTIC_ACTIVE_SEARCH_PROFILES)
    _, candidates = read_rows(diagnostic_active_search_dir / DIAGNOSTIC_ACTIVE_SEARCH_CANDIDATES)
    if len(rows) != 972 or len(profiles) != 2916:
        raise RuntimeError("DiagnosticActiveSearch result does not contain 972 environments and three profiles each")
    ...
    write_rows(
        output_dir / "diagnostic_active_search_representative_environments.csv",
...
def render_report(method_comparison: Mapping[str, object], diagnostic_active_search: Mapping[str, object]) -> str:
    comparisons = method_comparison["comparisons"]
    strongest = diagnostic_active_search["strongest_rr_environment"]
    diagnostic_active_search_n = int(diagnostic_active_search["environments"])
...
<body><main>
  ...
  <h2>Key Results</h2>
  <div class="grid">
    <article class="card"><span class="tag">MethodComparison precision</span><span class="number">{fmt(float(method_comparison['max_paired_utility_ci95_half_width']))}</span><p>Maximum paired 95% CI half-width across canonical method comparisons, below one utility unit.</p></article>
    <article class="card"><span class="tag">DiagnosticActiveSearch environment test</span><span class="number">{diagnostic_active_search['manual_utility_above_equal_split_count']}/{diagnostic_active_search_n}</span><p>Environments where manual active search produced higher utility than equal split ({pct(int(diagnostic_active_search['manual_utility_above_equal_split_count']), diagnostic_active_search_n)}).</p></article>
    <article class="card"><span class="tag red">DiagnosticActiveSearch discovery test</span><span class="number">0/{diagnostic_active_search_n}</span><p>Environments where RR reached the strict true equal-outcome threshold of 0.90.</p></article>
  </div>

  <h2>method comparison: Approximation Methods</h2>
  <p>The completed comparison contains {method_comparison['episode_rows']:,} episode rows, {method_comparison['method_summary_rows']} method summaries, and {method_comparison['environments']} environments. All canonical comparisons use the same 1,200 true states and observation streams; fingerprint mismatches: {method_comparison['common_randomness_mismatches']}. The largest marginal mean-utility CI half-width is {fmt(float(method_comparison['max_marginal_mean_utility_ci95_half_width']))}; pairing reduces the maximum difference-CI half-width to {fmt(float(method_comparison['max_paired_utility_ci95_half_width']))}. Sampling consumes time rather than a fixed utility penalty, so one sample does not have one constant utility-unit cost.</p>
  ...
  <p class="note"><strong>Interpretation.</strong> The 10x run makes method differences much more precise, but the canonical DP point estimate remains below myopic VOI in {method_comparison['canonical_dp_point_estimate_below_myopic_environments']} of 14 environments. At least one tested DP configuration exceeds myopic VOI in all 14 environments, but selecting the best of 48 DP settings on the same episodes is exploratory and upward-biased. This supports approximation sensitivity, not a claim that DP is exactly optimal.</p>

  <h2>diagnostic active search: Diagnostic Active Search</h2>
  <p>The diagnostic grid contains {diagnostic_active_search_n} environments and three policies per environment: RR/myopic VOI, manual active-search equal outcome, and equal split. The manual policy used six observations in every environment. It had higher utility than equal split in {diagnostic_active_search['manual_utility_above_equal_split_count']}/{diagnostic_active_search_n} environments, and {diagnostic_active_search['manual_candidate_rule_count']}/{diagnostic_active_search_n} passed the predefined strict candidate rule. RR had higher utility than the manual policy in {diagnostic_active_search['rr_utility_above_manual_count']}/{diagnostic_active_search_n} environments.</p>
  <div class="grid">
    <article class="card"><span class="number">{fmt(float(diagnostic_active_search['mean_manual_minus_equal_split_utility']))}</span><p>Mean manual active-search utility advantage over equal split.</p></article>
    <article class="card"><span class="number">{fmt(float(diagnostic_active_search['mean_rr_minus_manual_utility']))}</span><p>Mean RR utility advantage over manual active search.</p></article>
    <article class="card"><span class="number">{fmt(float(diagnostic_active_search['mean_rr_sample_count']))}</span><p>Mean RR sample count, compared with {fmt(float(diagnostic_active_search['mean_manual_sample_count']))} for the manual policy.</p></article>
  </div>
  <table><thead><tr><th>Behavior</th><th>Manual active search</th><th>RR approximation</th></tr></thead><tbody>
    <tr><td>Mean true equal-outcome rate</td><td>{fmt(float(diagnostic_active_search['mean_manual_true_equal_outcome_rate']))}</td><td>{fmt(float(diagnostic_active_search['mean_rr_true_equal_outcome_rate']))}</td></tr>
    <tr><td>Mean closer-to-true-equal rate</td><td>{fmt(float(diagnostic_active_search['mean_manual_closer_to_true_equal_rate']))}</td><td>{fmt(float(diagnostic_active_search['mean_rr_closer_to_true_equal_rate']))}</td></tr>
    <tr><td>Mean outcome distance to true equal outcome</td><td>{fmt(float(diagnostic_active_search['mean_manual_outcome_distance_to_true_equal']))}</td><td>{fmt(float(diagnostic_active_search['mean_rr_outcome_distance_to_true_equal']))}</td></tr>
    <tr><td>Environments above 0.90 true equal outcome</td><td>Diagnostic benchmark</td><td>{diagnostic_active_search['rr_true_equal_outcome_rate_ge_0_9_count']} / {diagnostic_active_search_n}</td></tr>
    <tr><td>Environments closer to true equal outcome than equal split in most episodes</td><td>Not the primary selection test</td><td>{diagnostic_active_search['rr_closer_to_true_equal_rate_gt_0_5_count']} / {diagnostic_active_search_n}</td></tr>
  </tbody></table>
  <p class="note"><strong>Where the manual policy did not help.</strong> Manual active search failed to outperform equal split in {diagnostic_active_search['manual_nonpositive_utility_advantage_count']}/{diagnostic_active_search_n} environments. These failures were concentrated in high-need and scarce-time cases: {diagnostic_active_search['manual_loss_mu_need_counts']['55.0']}/87 had <code>mu_need = 55</code>, and {diagnostic_active_search['manual_loss_total_time_counts']['80.0']}/87 had <code>total_time = 80</code>.</p>
  ...
  <h2>Conclusion</h2>
  <p><strong>The diagnostic environment construction succeeded:</strong> manual active search improved utility over equal split in {diagnostic_active_search['manual_utility_above_equal_split_count']} environments and reduced the true outcome gap in all {diagnostic_active_search_n}.</p>
  <p><strong>The current RR approximation did not reproduce the manual policy's strong equal-outcome behavior:</strong> it sampled actively and achieved higher utility, but its maximum true equal-outcome rate was {fmt(float(diagnostic_active_search['maximum_rr_true_equal_outcome_rate']))}. The present evidence therefore does not yet establish a discovered multi-step resource-rational equal-outcome heuristic.</p>
  ...
  <p><a href="supporting_data/method_comparison_paired_method_comparisons.csv">MethodComparison paired comparisons</a> · <a href="supporting_data/method_comparison_dp_sensitivity.csv">MethodComparison DP sensitivity</a> · <a href="supporting_data/diagnostic_active_search_representative_environments.csv">DiagnosticActiveSearch representative environments</a> · <a href="supporting_data/analysis_summary.json">Analysis summary</a></p>
  <p class="muted">The supporting folder also includes the complete 700-row MethodComparison method summary, the complete DiagnosticActiveSearch aggregate tables, and run-validation evidence. The 840,000-row MethodComparison episode file is retained in the project result archive rather than duplicated in this professor-facing folder.</p>
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--method_comparison-dir", type=Path, required=True)
    parser.add_argument("--diagnostic_active_search-dir", type=Path, required=True)
...
def main() -> int:
    ...
    method_comparison = analyze_method_comparison(args.method_comparison_dir, supporting_dir)
    diagnostic_active_search = analyze_active_search_diagnostic(
        args.diagnostic_active_search_dir,
        supporting_dir,
    )
    for source, destination in (
        (args.method_comparison_dir / METHOD_COMPARISON_SUMMARY, supporting_dir / METHOD_COMPARISON_SUMMARY),
        (
            args.method_comparison_dir / "method_comparison_combine_status.csv",
            supporting_dir / "method_comparison_combine_status.csv",
        ),
        (
            args.method_comparison_dir / "method_comparison_episode_array_progress.json",
            supporting_dir / "method_comparison_episode_array_progress.json",
        ),
        (
            args.method_comparison_dir / "recovery" / "stage_evidence_recovery.json",
            supporting_dir / "method_comparison_stage_evidence_recovery.json",
        ),
        (args.diagnostic_active_search_dir / DIAGNOSTIC_ACTIVE_SEARCH_SUMMARY, supporting_dir / DIAGNOSTIC_ACTIVE_SEARCH_SUMMARY),
        (args.diagnostic_active_search_dir / DIAGNOSTIC_ACTIVE_SEARCH_PROFILES, supporting_dir / DIAGNOSTIC_ACTIVE_SEARCH_PROFILES),
        (args.diagnostic_active_search_dir / DIAGNOSTIC_ACTIVE_SEARCH_CANDIDATES, supporting_dir / DIAGNOSTIC_ACTIVE_SEARCH_CANDIDATES),
        (args.diagnostic_active_search_dir / "diagnostic_active_search_array_progress.json", supporting_dir / "diagnostic_active_search_array_progress.json"),
        (args.diagnostic_active_search_dir / "diagnostic_active_search_array_manifest.json", supporting_dir / "diagnostic_active_search_array_manifest.json"),
    ):
        if source.is_file():
            shutil.copy2(source, destination)
    summary = {"method_comparison": method_comparison, "diagnostic_active_search": diagnostic_active_search}
    (supporting_dir / "analysis_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (args.output_dir / "index.html").write_text(render_report(method_comparison, diagnostic_active_search), encoding="utf-8")
