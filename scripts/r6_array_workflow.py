#!/usr/bin/env python3
"""Create, execute, monitor, and strictly collect R6 array shards."""
...
from src.experiments.r6 import (  # noqa: E402
...
SCHEMA_VERSION = 1
MANIFEST_FILENAME = "r6_manifest.json"
PROGRESS_FILENAME = "r6_progress.json"
VALIDATION_FILENAME = "r6_validation.json"
ARTIFACT_INDEX_FILENAME = "r6_artifact_index.json"
COMPLETION_FILENAME = "COMPLETED.json"
SCHEDULER_FILENAME = "r6_scheduler.json"
COLLECTION_LOCK_FILENAME = ".r6_collection.lock"
TASK_LOCK_FILENAME = ".r6_task.lock"
...
EPISODE_OUTPUTS = {
    ANALYSIS_FOUR_WAY: "r6_four_way_episodes.csv",
    ANALYSIS_SIGMA: "r6_sigma_need_episodes.csv",
    ANALYSIS_FIXED: "r6_fixed_total_need_episodes.csv",
}
SUMMARY_OUTPUTS = (
    "r6_four_way_policy_summary.csv",
    "r6_four_way_paired_comparisons.csv",
    "r6_sigma_need_environment_summary.csv",
    "r6_sigma_need_gap_strata.csv",
    "r6_fixed_total_need_summary.csv",
)
...
IMPLEMENTATION_SOURCES = (
    "configs/r6_primary_environments.json",
    "configs/r6_output_schemas.json",
    "scripts/r6_array_workflow.py",
    "scripts/submit_hoffman2_round6_array.sh",
    "src/experiments/r5.py",
    "src/experiments/r6.py",
...
def load_schema_contract(path: Path) -> Dict[str, List[str]]:
    ...
    if payload.get("schema_version") != 1:
        raise RuntimeError("Unsupported R6 output-schema contract")
    schemas = {str(key): list(value) for key, value in payload["schemas"].items()}
    if set(schemas) != set(ALL_OUTPUTS):
        raise RuntimeError("R6 output-schema contract has missing or extra tables")
    return schemas
...
def _load_frozen_inputs(
    ...
    if [str(row["environment"]) for row in frozen_rows] != expected_names:
        raise RuntimeError("Frozen R6 environments do not match deterministic R4 selection")
    checked_fields = (
        "mu_need",
        "sigma_need",
        "sigma_sample",
    ...
    for source, frozen_row in zip(selected_rows, frozen_rows):
        if int(float(source["grid_index"])) != int(frozen_row["source_grid_index"]):
            raise RuntimeError("Frozen R6 grid index does not match its R4 source row")
        for field in checked_fields:
            if float(source[field]) != float(frozen_row["config"][field]):
                raise RuntimeError(f"Frozen R6 parameter mismatch: {field}")
...
def _validate_partition(episodes: int, episodes_per_task: int) -> None:
    ...
    if episodes % 2 or episodes_per_task % 2:
        raise ValueError("R6 fixed-total orientation requires even episode partitions")
...
def build_tasks(
    ...
    for difference_index, difference in enumerate(R6_FIXED_TOTAL_NEED_DIFFERENCES):
        for start in range(0, episodes, episodes_per_task):
            add_task(
                ANALYSIS_FIXED,
                start,
...
def _expected_row_counts(environment_count: int, episodes: int, sigma_count: int) -> Dict[str, int]:
    four_way_strata = 9
    sigma_strata = 2 + 2 + 4 + (len(R6_DEFAULT_GAP_BIN_EDGES) - 1) + (
        len(R6_DEFAULT_TOTAL_NEED_BIN_EDGES) - 1
    )
    fixed_strata = 1 + 2 + 2 + 4
    fixed_count = len(R6_FIXED_TOTAL_NEED_DIFFERENCES)
    return {
        "r6_four_way_episodes.csv": environment_count * episodes * 4,
        "r6_four_way_policy_summary.csv": environment_count * four_way_strata * 4,
        "r6_four_way_paired_comparisons.csv": environment_count * four_way_strata * 7,
        "r6_sigma_need_episodes.csv": sigma_count * episodes,
        "r6_sigma_need_environment_summary.csv": sigma_count,
        "r6_sigma_need_gap_strata.csv": sigma_count * sigma_strata,
        "r6_fixed_total_need_episodes.csv": fixed_count * episodes,
        "r6_fixed_total_need_summary.csv": fixed_count * fixed_strata + 1,
    }
...
def build_manifest(
    *,
    ...
    if analysis_mode == "serious" and (
        ...
        raise ValueError(
            "Serious R6 settings are frozen at 1200 episodes, 10 episodes/task, "
    ...
    if analysis_mode == "smoke" and (
        ...
        raise ValueError(
            "Scheduled R6 smoke is frozen at two episodes, 500 draws, grid 4001, "
            "and namespace 70000000"
        )
    if len(selected_configs) != 3 or len(selected_rows) != 3:
        raise ValueError("R6 requires exactly three frozen primary environments")
    _validate_partition(episodes, episodes_per_task)
    ...
    manifest: Dict[str, object] = {
        ...
        "workflow": "round6_serious_hoffman2_array",
        ...
        "r4_input_dir": str(r4_dir.resolve()),
        "r4_input_hashes": dict(r4_hashes),
...
def load_manifest(path: Path) -> Dict[str, object]:
    ...
    if claimed_hash != actual_hash:
        raise RuntimeError("R6 manifest hash mismatch")
    if manifest.get("schema_version") != SCHEMA_VERSION:
        raise RuntimeError("Unsupported R6 manifest schema")
    if manifest.get("workflow") != "round6_serious_hoffman2_array":
        raise RuntimeError("Unexpected R6 workflow")
    if manifest.get("analysis_mode") == "serious":
        ...
        if (
            ...
            raise RuntimeError("Serious R6 manifest settings changed")
    if manifest.get("analysis_mode") == "smoke":
        ...
        if (
            ...
            raise RuntimeError("Scheduled R6 smoke settings changed")
    ...
    if manifest["tasks"] != expected_tasks:
        raise RuntimeError("R6 task map is not the exact frozen Cartesian partition")
    return manifest


def verify_execution_checkout(manifest: Mapping[str, object]) -> None:
    if git_commit() != manifest["git_commit"]:
        raise RuntimeError("Execution checkout does not match the frozen R6 commit")
...
def execute_task_rows(
    ...
    if analysis == ANALYSIS_FOUR_WAY:
        ...
        rows = evaluate_r6_four_way_environment(
        ...
        if analysis == ANALYSIS_SIGMA:
            rows = evaluate_r6_sigma_need_sweep(
                str(entry["environment"]),
                config,
            ...
            rows = evaluate_r6_fixed_total_need_diagnostic(
                str(entry["environment"]),
                config,
            ...
            raise ValueError(f"Unknown R6 task analysis: {analysis}")
...
def _validate_episode_domains(
    ...
    for row_index, row in enumerate(rows):
        ...
        if analysis == ANALYSIS_FOUR_WAY:
            if policy not in R6_POLICY_ORDER:
                raise RuntimeError(f"Unknown policy in {label}")
            if policy in (R6_POLICY_EQUAL_SPLIT, R6_POLICY_ORACLE) and counts[
                "online_sample_count"
            ] != 0:
                raise RuntimeError(f"Zero-information benchmark sampled in {label}")
            if policy == R6_POLICY_EQUAL_SPLIT and abs(allocation - 0.5) > 1e-12:
                raise RuntimeError(f"Equal-split benchmark is not 50/50 in {label}")
            if policy == R6_POLICY_MANUAL and (
...
def validate_episode_rows(
    ...
    if analysis == ANALYSIS_FOUR_WAY:
        ...
        for key, episode_rows in grouped.items():
            if {str(row["policy"]) for row in episode_rows} != set(R6_POLICY_ORDER):
...
def _sort_episode_rows(analysis: str, rows: Iterable[Mapping[str, object]]) -> List[Mapping[str, object]]:
    policy_order = {policy: index for index, policy in enumerate(R6_POLICY_ORDER)}
    if analysis == ANALYSIS_FOUR_WAY:
        return sorted(
            rows,
            key=lambda row: (
...
def exclusive_collection_lock(output_dir: Path):
    ...
    try:
        ...
        raise RuntimeError(f"Another R6 collector holds {lock_path}") from error
    try:
...
def _validate_summary_rows(filename: str, rows: Sequence[Mapping[str, object]]) -> None:
    for row_index, row in enumerate(rows):
        count_field = "n_pairs" if filename == "r6_four_way_paired_comparisons.csv" else "n_episodes"
...
def _validate_result_directory(
    ...
    if not result_dir.is_dir() or not artifact_path.is_file() or not validation_path.is_file():
        raise RuntimeError("R6 promoted result directory is incomplete")
    ...
    if artifact_index.get("manifest_hash") != manifest["manifest_hash"]:
        raise RuntimeError("R6 artifact index has a stale manifest fingerprint")
    if validation.get("manifest_hash") != manifest["manifest_hash"]:
        raise RuntimeError("R6 validation has a stale manifest fingerprint")
    if artifact_index.get("manifest_sha256") != sha256_file(manifest_path):
        raise RuntimeError("R6 artifact index has a stale manifest hash")
    if validation.get("manifest_sha256") != sha256_file(manifest_path):
        raise RuntimeError("R6 validation has a stale manifest hash")
    if validation.get("artifact_index_sha256") != sha256_file(artifact_path):
        raise RuntimeError("R6 validation has a stale artifact-index hash")
    if validation.get("output_metadata") != artifact_index.get("outputs"):
        raise RuntimeError("R6 validation and artifact index disagree")
    ...
    for filename in ALL_OUTPUTS:
        ...
        if not path.is_file() or metadata_row is None:
            raise RuntimeError(f"Missing promoted R6 output: {filename}")
        if sha256_file(path) != metadata_row["sha256"]:
            raise RuntimeError(f"Promoted R6 output hash mismatch: {filename}")
        fields = read_csv_schema(path)
        if fields != manifest["expected_output_schemas"][filename]:
            raise RuntimeError(f"Promoted R6 output schema mismatch: {filename}")
        if schema_hash(fields) != metadata_row["schema_sha256"]:
            raise RuntimeError(f"Promoted R6 output schema hash mismatch: {filename}")
        rows = read_csv(path)
        if len(rows) != int(manifest["expected_row_counts"][filename]):
            raise RuntimeError(f"Promoted R6 output row-count mismatch: {filename}")
...
def _collect_unlocked(manifest_path: Path) -> Dict[str, object]:
    ...
    four_policy, four_paired = summarize_r6_four_way(by_analysis[ANALYSIS_FOUR_WAY])
    sigma_environment, sigma_strata = summarize_r6_sigma_need_sweep(by_analysis[ANALYSIS_SIGMA])
    fixed_summary = summarize_r6_fixed_total_need_diagnostic(by_analysis[ANALYSIS_FIXED])
    ...
    tables: Dict[str, Sequence[Mapping[str, object]]] = {
        "r6_four_way_episodes.csv": by_analysis[ANALYSIS_FOUR_WAY],
        "r6_four_way_policy_summary.csv": four_policy,
        "r6_four_way_paired_comparisons.csv": four_paired,
        "r6_sigma_need_episodes.csv": by_analysis[ANALYSIS_SIGMA],
        "r6_sigma_need_environment_summary.csv": sigma_environment,
        "r6_sigma_need_gap_strata.csv": sigma_strata,
        "r6_fixed_total_need_episodes.csv": by_analysis[ANALYSIS_FIXED],
        "r6_fixed_total_need_summary.csv": fixed_summary,
    }
    staging_dir = output_dir / f".r6_collection_staging_{str(manifest['manifest_hash'])[:12]}"
    ...
    result_version = (
        f"r6_result_{str(manifest['manifest_hash'])[:12]}_"
...
def validate_final_tree(manifest_path: Path) -> Dict[str, object]:
    ...
    if not completion_path.is_file():
        raise RuntimeError("R6 final tree is missing its completion marker")
    ...
    if (
        ...
        raise RuntimeError("R6 completion points outside the result-version directory")
    ...
    if completion.get("status") != expected_status:
        raise RuntimeError("R6 completion status does not match manifest mode")
    if bool(completion.get("scientific_completion")) != expected_scientific:
        raise RuntimeError("R6 completion scientific flag does not match manifest mode")
    if completion.get("manifest_sha256") != sha256_file(manifest_path):
        raise RuntimeError("R6 completion has a stale manifest hash")
    if completion.get("validation_sha256") != sha256_file(validation_path):
        raise RuntimeError("R6 completion has a stale validation hash")
    if completion.get("artifact_index_sha256") != sha256_file(artifact_path):
        raise RuntimeError("R6 completion has a stale artifact-index hash")
    if completion.get("output_metadata") != validated["output_metadata"]:
        raise RuntimeError("R6 completion output metadata mismatch")
    return {
        "status": "valid",
...
def create_manifest_from_args(args: argparse.Namespace) -> Path:
    ...
    if not git_is_clean():
        raise RuntimeError("R6 manifest creation requires a clean committed worktree")
    output_dir = args.output_dir.resolve()
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(f"R6 output directory is not empty: {output_dir}")
    selected_rows, selected_configs, r4_hashes = _load_frozen_inputs(
        args.r4_dir.resolve(),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Reproducible R6 Hoffman2 array workflow")
    ...
    create.add_argument(
        "--frozen-config", type=Path, default=Path("configs/r6_primary_environments.json")
    )
    create.add_argument(
        "--schema-contract", type=Path, default=Path("configs/r6_output_schemas.json")
    )
