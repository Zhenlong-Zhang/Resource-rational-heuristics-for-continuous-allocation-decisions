...
from src.experiments.active_search_evaluation import (  # noqa: E402
    summarize_active_search_oracle_map,
    summarize_active_search_rr_environments,
)
from src.experiments.sweeps import build_active_search_oat_configs, build_active_search_sampling_cost_configs  # noqa: E402
            ...
            slices.append(
                {
                    ...
                    "active_search_joint_0_8_0_8": 1.0 if true_rate >= 0.8 and closer_rate >= 0.8 else 0.0,
...
def _formal_sweep_row(
    ...
    fields = (
        ...
        "active_search_joint_discovery_candidate",
    )
    ...
    result["active_search_joint_0_8_0_8"] = 1.0 if true_rate >= 0.8 and closer_rate >= 0.8 else 0.0
    return result
...
def build_solver_comparison_rows(
    ...
    for environment in environments:
        ...
        myopic_summary = summarize_active_search_rr_environments(
            [pair[0] for pair in environment_pairs]
        )[0]
        dp_summary = summarize_active_search_rr_environments(
        ...
        for prefix, summary in (("myopic", myopic_summary), ("dp", dp_summary)):
            for field in (
                ...
                "sample_count_at_least_6_rate",
                "active_search_joint_confirmed",
            ):
                row[f"{prefix}_{field}"] = summary[field]
        row["dp_active_search_joint_confirmed"] = 1.0 if (
            float(dp_summary["active_search_joint_confirmed"]) >= 0.5
...
def analyze_solver_comparison(args) -> None:
    ...
    write_csv(output_dir / "active_search_solver_comparison.csv", rows)
    write_solver_comparison_svg(
        output_dir / "figures" / "active_search_solver_paired_utility.svg",
        rows,
    )
    summary = {
    ...
    write_json(output_dir / "active_search_solver_comparison_summary.json", summary)
...
def analyze_formal_summaries(args) -> None:
    ...
    output_dir = Path(args.output_dir)
    write_csv(output_dir / "active_search_formal_sweep_long.csv", sweep_rows)
    write_csv(output_dir / "active_search_fixed_budget_evidence.csv", fixed_evidence)
    for feature in ("sample_time_cost_percent",) + FORMAL_OAT_FEATURES:
        ...
        if feature_rows:
            write_formal_feature_svg(
                output_dir / "figures" / f"active_search_formal_{feature}.svg",
                feature,
                feature_rows,
            )
    write_fixed_budget_svg(output_dir / "figures" / "active_search_fixed_budget_utility.svg", fixed_rows)
    joint_rows = [row for row in sweep_rows if float(row["active_search_joint_0_8_0_8"]) >= 0.5]
    ...
    write_json(output_dir / "active_search_formal_summary.json", result)
...
def analyze_existing(args) -> None:
    ...
    output_dir = Path(args.output_dir)
    write_csv(output_dir / "active_search_existing_oat_slices.csv", slices)
    for feature in EXISTING_OAT_FEATURES:
        ...
        if len(feature_rows) > 1:
            write_existing_oat_svg(
                output_dir / f"active_search_existing_oat_{feature}.svg",
                feature,
                feature_rows,
            )
    ...
    write_json(output_dir / "active_search_existing_reanalysis_summary.json", summary)
    print(json.dumps(summary, sort_keys=True))
...
def select_oracle_anchors(
    ...
    eligible = [
        ...
        if float(row["active_search_joint_oracle_candidate"]) >= 0.5
...
def analyze_oracle(args) -> None:
    ...
    summaries = summarize_active_search_oracle_map(rows)
    manifest = load_manifest(Path(args.manifest))
    configs = environment_configs(manifest)
    anchors = select_oracle_anchors(summaries, configs, args.anchor_count)

    output_dir = Path(args.output_dir)
    write_csv(output_dir / "active_search_oracle_sign_summary.csv", summaries)
    write_json(
        output_dir / "active_search_oracle_anchors.json",
        config_payload(
            anchors,
    ...
    sampling = build_active_search_sampling_cost_configs(anchors)
    write_json(
        output_dir / "active_search_sampling_cost_sweep_configs.json",
        config_payload(sampling, "active_search_oracle_anchors.json", "prespecified sample-cost percentages"),
    )
    ...
    for anchor_name, anchor in anchors:
        for feature, _, name, config in build_active_search_oat_configs(anchor_name, anchor, oat_values):
            oat_configs.append((f"{feature}_{name}", config))
    write_json(
        output_dir / "active_search_oat_sweep_configs.json",
        config_payload(oat_configs, "active_search_oracle_anchors.json", "one factor at a time"),
    )

    candidates = [row for row in summaries if float(row["active_search_joint_oracle_candidate"]) >= 0.5]
    result = {
    ...
    write_json(output_dir / "active_search_oracle_analysis.json", result)
    print(json.dumps(result, sort_keys=True))
...
def select_confirmation(args) -> None:
    ...
    eligible = [
        row
        for row in summaries
        if float(row["active_search_joint_discovery_candidate"]) >= 0.5
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Analyze and freeze ActiveSearch discovery outputs")
