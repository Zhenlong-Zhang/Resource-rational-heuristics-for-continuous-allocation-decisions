...
# Submit the three R6 families from one immutable manifest.
...
R4_DIR="${R4_DIR:-results/r4_diagnostic_active_search_server_20260714_array486}"
...
OUTPUT_DIR="${OUTPUT_DIR:-results/r6_${RUN_MODE}_$(date +%Y%m%d_%H%M%S)}"
if [[ "${OUTPUT_DIR}" = /* ]]; then
...
if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
  echo "R6 submission requires a clean committed worktree." >&2
  exit 1
fi

"${PYTHON_BIN}" scripts/r6_array_workflow.py create \
...
MANIFEST="${OUTPUT_DIR_ABS}/r6_manifest.json"
...
if not indices or indices != list(range(min(indices), max(indices) + 1)):
    raise SystemExit("non-contiguous R6 task family")
...
submit_family() {
  ...
  local task_count="$3"
  local throttle="$4"
  local job_file="${SCHEDULER_DIR}/r6_${RUN_MODE}_${family}_array.job"
  local job_name="r6${MODE_TAG}${family:0:1}${RUN_TOKEN}"
  cat > "${job_file}" <<EOF
#!/usr/bin/env bash
#$ -cwd
#$ -N r6${MODE_TAG}_${family}
#$ -q ${QUEUE}
#$ -j y
#$ -o ${LOG_DIR}/r6_${RUN_MODE}_${family}.\$JOB_ID.\$TASK_ID.log
#$ -l h_rt=${TASK_H_RT}
...
"${PYTHON_BIN}" scripts/r6_array_workflow.py run-task \
  --manifest "${MANIFEST}" \
  --task-index "\${GLOBAL_TASK_INDEX}"
EOF
  chmod +x "${job_file}"
  r6_submit_job "${family}" "${job_name}" "${job_file}"
  LAST_JOB_ID="${R6_LAST_JOB_ID}"
}
...
COLLECT_JOB="${SCHEDULER_DIR}/r6_${RUN_MODE}_collect.job"
cat > "${COLLECT_JOB}" <<EOF
#!/usr/bin/env bash
#$ -cwd
#$ -N r6${MODE_TAG}_collect
#$ -q ${QUEUE}
#$ -j y
#$ -o ${LOG_DIR}/r6_${RUN_MODE}_collect.\$JOB_ID.log
#$ -l h_rt=${COLLECT_H_RT}
...
export LC_ALL=C
cd "${PROJECT_ROOT}"
"${PYTHON_BIN}" scripts/r6_array_workflow.py collect --manifest "${MANIFEST}"
EOF
chmod +x "${COLLECT_JOB}"
COLLECT_JOB_NAME="r6${MODE_TAG}c${RUN_TOKEN}"
r6_submit_job "collector" "${COLLECT_JOB_NAME}" "${COLLECT_JOB}"
COLLECT_JOB_ID="${R6_LAST_JOB_ID}"

"${PYTHON_BIN}" scripts/r6_array_workflow.py record-jobs \
  --manifest "${MANIFEST}" \
...
printf 'progress: %s scripts/r6_array_workflow.py progress --manifest %s\n' \
  "${PYTHON_BIN}" "${MANIFEST}"
