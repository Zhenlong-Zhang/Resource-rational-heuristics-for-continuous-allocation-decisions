...
WORKFLOW_RUN_DIR="${WORKFLOW_RUN_DIR:-results/method_comparison_approximation_methods_episode_array}"
MANIFEST="${MANIFEST:-${WORKFLOW_RUN_DIR}/method_comparison_episode_array_manifest.json}"
...
if [[ "${task_count}" -eq 0 ]]; then
  ...
  "${PYTHON_BIN}" scripts/method_comparison_episode_workflow.py status --manifest "${MANIFEST}"
  exit 0
fi
...
submission_json=$("${PYTHON_BIN}" scripts/method_comparison_episode_workflow.py submission \
...
rollback_submission() {
  ...
  set +e
  current=$("${PYTHON_BIN}" scripts/method_comparison_episode_workflow.py submission \
...
' 2>/dev/null)
  ...
  "${PYTHON_BIN}" scripts/method_comparison_episode_workflow.py submission \
...
PY
  ...
  lane_job_file="${JOB_DIR}/method_comparison_episode_array_lane_${lane_id}.job"
  cat > "${lane_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o ${LOG_DIR}/method_comparison_episode_array_lane${lane_id}.\$JOB_ID.\$TASK_ID.log
#$ -j y
#$ -l h_rt=${TASK_H_RT},h_data=${TASK_H_DATA}
#$ -N rr_method_comparison_e${lane_id}
...
"${PYTHON_BIN}" scripts/method_comparison_episode_workflow.py run-shard \
  --manifest "${MANIFEST}" \
  --task-id "\${TASK_ID}" \
...
collector_job_file="${JOB_DIR}/method_comparison_episode_array_collector.job"
cat > "${collector_job_file}" <<EOF
#!/bin/bash
#$ -S /bin/bash
#$ -cwd
#$ -o ${LOG_DIR}/method_comparison_episode_array_collector.\$JOB_ID.log
#$ -j y
#$ -l h_rt=${COLLECT_H_RT},h_data=${COLLECT_H_DATA}
#$ -N rr_method_comparison_ecol

export LC_ALL=C
export LANG=C
set -euo pipefail

"${PYTHON_BIN}" scripts/method_comparison_episode_workflow.py collect \
  --manifest "${MANIFEST}" \
  --promote
"${PYTHON_BIN}" scripts/method_comparison_episode_workflow.py status --manifest "${MANIFEST}"
EOF
...
echo "Status: ${PYTHON_BIN} scripts/method_comparison_episode_workflow.py status --manifest ${MANIFEST}"
