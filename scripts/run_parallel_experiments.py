...
RESULT_SET_BY_FILE = {
    ...
    "active_search_diagnostic_policy_profiles.csv": "active_search_diagnostic_policy_profiles",
    "active_search_diagnostic_environment_summary.csv": "active_search_diagnostic_environment_summary",
    "active_search_diagnostic_manual_advantage_candidates.csv": "active_search_diagnostic_manual_advantage_candidates",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sections",
        default="all",
        help="Comma-separated: step7,sweeps,regime_grid,active_search_diagnostic,dp,gh or all.",
    )
...
def build_tasks(args: argparse.Namespace, run_dir: Path) -> List[TaskSpec]:
    ...
    if "active_search_diagnostic" in sections:
        if args.regime_grid_chunks <= 0:
            raise ValueError("--regime-grid-chunks must be positive")
        selected_grids = args.regime_grid or ["active_search_benchmark"]
...
def main() -> int:
    ...
    run_dir = PROJECT_ROOT / (
        args.output_dir
        or f"results/baseline_experiment_parallel_{args.preset}_{timestamp}"
    )
    ...
    print(f"Parallel BaselineExperiment run directory: {run_dir}")
