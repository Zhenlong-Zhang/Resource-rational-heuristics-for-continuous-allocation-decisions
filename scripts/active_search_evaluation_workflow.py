...
from src.experiments.sweeps import (  # noqa: E402
    build_active_search_oracle_map_configs,
    build_active_search_six_sample_configs,
)
...
def family_configs(family: str) -> List[tuple[str, EnvironmentConfig]]:
    if family == "oracle":
        return build_active_search_oracle_map_configs()
    if family == "six_sample":
        return build_active_search_six_sample_configs()
    raise ValueError(f"Unknown ActiveSearch family: {family}")
...
def create_manifest(args) -> None:
    ...
    write_json(output_dir / "active_search_manifest.json", manifest)
    write_json(
        output_dir / "active_search_progress.json",
        {"created_at": utc_now(), "total_tasks": len(tasks), "completed_tasks": 0},
    )
    print(canonical_json({"manifest": str(output_dir / "active_search_manifest.json"), "tasks": len(tasks)}))
...
def progress(args) -> None:
    ...
    write_json(manifest_path.parent / "active_search_progress.json", value)
...
def collect(args) -> None:
    ...
    if manifest["family"] == "oracle":
        rows_name = "active_search_oracle_episodes.csv"
        summary_name = "active_search_oracle_environment_summary.csv"
        summaries = summarize_active_search_oracle_map(all_rows)
    elif manifest["family"] == "fixed_budget":
        rows_name = "active_search_fixed_budget_episodes.csv"
        summary_name = "active_search_fixed_budget_summary.csv"
        summaries = summarize_active_search_fixed_sampling_budgets(all_rows)
    else:
        rows_name = "active_search_rr_episodes.csv"
        summary_name = "active_search_rr_environment_summary.csv"
        summaries = summarize_active_search_rr_environments(all_rows)
    ...
    candidates = [] if manifest["family"] == "fixed_budget" else [
        ...
        if float(row.get("active_search_joint_oracle_candidate", row.get("active_search_joint_discovery_candidate", 0.0))) >= 0.5
    ]
    write_csv(output_dir / "active_search_joint_candidates.csv", candidates)
    completed = {
        "completed_at": utc_now(),
        "family": manifest["family"],
        "git_commit": manifest["git_commit"],
