...
OUTPUT_DIR="${OUTPUT_DIR:-results/active_search_${FAMILY}_$(date +%Y%m%d_%H%M%S)}"
...
CREATE_ARGS=(
  scripts/active_search_evaluation_workflow.py create
  --family "${FAMILY}"
...
MANIFEST="${OUTPUT_DIR}/active_search_manifest.json"
...
ARRAY_JOB="jobs/active_search_${FAMILY}_array.job"
cat > "${ARRAY_JOB}" <<EOF
#!/usr/bin/env bash
#$ -cwd
#$ -N active_search_${FAMILY}
#$ -q ${QUEUE}
#$ -j y
#$ -o logs/active_search_${FAMILY}.\$JOB_ID.\$TASK_ID.log
#$ -l h_rt=${TASK_TIME}
...
export LC_ALL=C
cd "${PROJECT_ROOT}"
python3 scripts/active_search_evaluation_workflow.py run-task \
  --manifest "${MANIFEST}" \
...
COLLECT_JOB="jobs/active_search_${FAMILY}_collect.job"
cat > "${COLLECT_JOB}" <<EOF
#!/usr/bin/env bash
#$ -cwd
#$ -N active_search_${FAMILY}_collect
#$ -q ${QUEUE}
#$ -j y
#$ -o logs/active_search_${FAMILY}_collect.\$JOB_ID.log
#$ -l h_rt=01:00:00
#$ -l h_data=4G
...
export LC_ALL=C
cd "${PROJECT_ROOT}"
python3 scripts/active_search_evaluation_workflow.py collect --manifest "${MANIFEST}"
EOF
