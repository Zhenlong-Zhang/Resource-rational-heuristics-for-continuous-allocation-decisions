...
MANIFEST="${MANIFEST:?Set MANIFEST to the frozen ActiveSearch manifest path}"
...
#!/usr/bin/env bash
#$ -cwd
#$ -N active_search_six_lane_${lane}
#$ -q ${QUEUE}
#$ -j y
#$ -o ${RUN_PROJECT_ROOT}/logs/active_search_six_lane_${lane}.\$JOB_ID.\$TASK_ID.log
#$ -l h_rt=${TASK_TIME}
...
#!/usr/bin/env bash
#$ -cwd
#$ -N active_search_six_lane_collect
#$ -q ${QUEUE}
#$ -j y
#$ -o ${RUN_PROJECT_ROOT}/logs/active_search_six_lane_collect.\$JOB_ID.log
#$ -l h_rt=01:00:00
#$ -l h_data=4G
#$ -hold_jid ${hold_ids}
set -euo pipefail
