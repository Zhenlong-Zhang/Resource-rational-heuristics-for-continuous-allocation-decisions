...
# Submit the frozen 90-case StrategyMapping quadrature diagnostic outside the clean checkout.
...
OUTPUT_DIR="${OUTPUT_DIR:-${SCRATCH_ROOT}/strategy_mapping_quadrature_diagnostic_$(date +%Y%m%d_%H%M%S)}"
...
"${PYTHON_BIN}" scripts/quadrature_validation_array.py create --output-dir "${OUTPUT_DIR_ABS}"
MANIFEST="${OUTPUT_DIR_ABS}/strategy_mapping_quadrature_diagnostic_manifest.json"
...
export LANG=C
cd "${PROJECT_ROOT_ABS}"
"${PYTHON_BIN}" scripts/quadrature_validation_array.py run-task \
  --manifest "${MANIFEST}" \
...
"${PYTHON_BIN}" scripts/quadrature_validation_array.py collect --manifest "${MANIFEST}"
EOF
chmod +x "${COLLECT_JOB}"
...
printf 'after qacct is available: %s scripts/quadrature_validation_array.py audit-qacct --manifest %s --job-id %s --job-id %s\n' \
  "${PYTHON_BIN}" "${MANIFEST}" "${ARRAY_JOB_ID}" "${COLLECT_JOB_ID}"
printf 'then finalize: %s scripts/quadrature_validation_array.py finalize --manifest %s\n' \
  "${PYTHON_BIN}" "${MANIFEST}"
